...
description: "Hosted ForeverBetter Wellness API plans, limits, billing, and self-hosting."
---

The hosted ForeverBetter Wellness API gives you managed authentication, private
...
continue. Stripe processes payment; ForeverBetter receives only verified
...
No. **Self-hosting is always an option.** The ForeverBetter Wellness API is
open source; run the API, workers, Postgres, object storage, and your data on
your own infrastructure. A hosted plan only pays for ForeverBetter-managed
...
Checkout. Self-hosting never requires a ForeverBetter payment method.
...
ForeverBetter Stripe account: **Standard $9.99**, **Builder $24.99**, and
...
`https://api.foreverbetter.xyz/billing/stripe/webhook`:
