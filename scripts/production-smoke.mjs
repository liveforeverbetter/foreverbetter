...
check("ready endpoint is healthy", async () => {
  ...
  if (body.service !== "foreverbetter-longevity-api") throw new Error(`unexpected service ${body.service}`);
});

check("OpenAPI branding is current", async () => {
  const body = await fetchJson("/openapi.json");
  if (body.info?.title !== "ForeverBetter Wellness API") throw new Error(`unexpected OpenAPI title ${body.info?.title}`);
...
check("agent manifest is current", async () => {
  ...
  if (body.name !== "ForeverBetter Wellness API") throw new Error(`unexpected agent manifest name ${body.name}`);
