...
**Install from a local folder (offline, air-gapped, or vendored in a repo):**
Drop the `wellnizz/` folder (this `SKILL.md`) into your agent's skills directory,
for example `~/.claude/skills/wellnizz/SKILL.md` or `~/.codex/skills/wellnizz/SKILL.md`.
The folder name is the skill name; no other files are required.

**Any other agent / manual:**
Paste this entire file into your agent's skill or system prompt. Point it at
`https://app.wellnizz.com` as the base URL. All auth, discovery, and endpoint
