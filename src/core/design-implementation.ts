...
export async function getDesignImplementation(id: string, baseUrl: string) {
  ...
  if (id === 'meridian') {
    ...
    return {
      schema_version: '1.0',
      id: 'meridian',
      name: 'Meridian full design-system handoff + wearable dashboard implementation',
      description: 'The complete Meridian component, token, template, and UI-kit specification plus a WHOOP-inspired wearable-performance dashboard. The dashboard is an independent wellnizz implementation, not affiliated with or endorsed by WHOOP.',
      format: 'design_system_handoff',
      entrypoint: specification.readme_path,
      files: specification.files,
      binary_assets: specification.binary_assets,
  ...
  throw new Error(`Unexpected design system: ${id}. Expected aperture, meridian, or foreverbetter.`);
}
