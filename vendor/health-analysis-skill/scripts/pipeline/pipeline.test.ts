...
import { computeAllPriorities, computePriority, getEvidenceWeight } from './priority_engine.js';
import { generateInsights } from './insight_engine.js';
import { generateProtocols } from './protocol_engine.js';
import { computeGLI, computeWeightedGLI, getGLIRating, computeCategoryGLI, computeWeightedCategoryGLI, computeEvidenceWeightedGLI } from './gli_engine.js';
import { computeHallmarkScores, getHallmarksForGene, getGenesForHallmark } from './hallmark_engine.js';
...
import { parseVEPOutput, parseVEPOutputSync, queryVEPForVariant, queryVEPForVariants, isVEPAvailable } from './vep_annotation.js';
...
import { buildWgsValidationCoverage, refreshWgsReadinessArtifacts } from './index.js';
import { analyzeWearables } from './wearable_engine.js';
import { buildMultiModalPlan } from './multimodal_engine.js';
import { parseBiomarkerCsv, parseBiomarkerText, parseWearableCsv, parseWearableJson } from './health_data_import.js';
...
describe('PRS consumer descriptions', () => {
  it('keeps coverage detail in metadata instead of repeating caution text for every trait', () => {
    ...
    const result = computePRS(new Map([[firstWeight.rsid, firstWeight.effect_allele]]));
    assert.ok(result.scores.length > 0);
    for (const score of result.scores) {
      assert.doesNotMatch(score.description, /interpret cautiously|configured PRS markers|directional rather than definitive|not a diagnosis/i);
...
const sampleTraitScores: Array<{ trait_id: string; score: number; confidence: number }> = [
  { trait_id: 'methylation', score: 35, confidence: 0.90 },
  { trait_id: 'caffeine_metabolism', score: 20, confidence: 0.85 },
  { trait_id: 'inflammation', score: 55, confidence: 0.80 },
  { trait_id: 'vitamin_d', score: 70, confidence: 0.75 },
  { trait_id: 'neuroplasticity', score: 60, confidence: 0.88 },
  { trait_id: 'dna_repair', score: 45, confidence: 0.70 },
...
describe('Graph Resolver', () => {
  ...
  it('should preserve trait scores and confidence', () => {
    ...
    for (const trait of enriched) {
      const original = sampleTraitScores.find(s => s.trait_id === trait.trait_id);
...
describe('Biomarker Engine', () => {
  it('should score biomarker readings independently by domain', () => {
    ...
    assert.ok(result.domains.some(d => d.id === 'cardiometabolic' && d.measured > 0));
    assert.ok(result.action_items.some(a => a.source_modalities.includes('biomarkers')));
  });
  ...
  it('should derive multi-marker biomarker ratios from available labs', () => {
    ...
    assert.ok(result.findings.some(f => f.id === 'non_hdl_c' && f.value === 170));
    assert.ok(result.findings.some(f => f.id === 'remnant_cholesterol' && f.value === 30));
    assert.ok(result.findings.some(f => f.id === 'apob_apoa1_ratio'));
    assert.ok(result.findings.some(f => f.id === 'transferrin_saturation' && f.value === 25));
    assert.ok(result.findings.some(f => f.id === 'vldl_c' && f.value === 27));
    assert.ok(result.lab_data?.some(f => f.id === 'total_cholesterol' && f.source_type === 'measured'));
    assert.ok(result.derived_biomarkers?.some(f => f.id === 'remnant_cholesterol' && f.formula.includes('total cholesterol')));
  ...
  it('should derive extended dashboard biomarkers from standard chemistry and CBC panels', () => {
    ...
    const derivedIds = new Set(result.derived_biomarkers?.map(item => item.id));
    for (const id of [
      'chol_hdl_ratio',
      'ldl_hdl_ratio',
    ...
    assert.ok(result.findings.some(f => f.id === 'tyg_index' && f.source_type === 'derived'));
    assert.ok(result.system_scores?.some(s => s.id === 'liver_function' && s.drivers.length > 0));
  });

  it('should preserve lab data separately from derived biomarkers', () => {
    const result = analyzeBiomarkers([
      { id: 'fasting_glucose', value: 97, unit: 'mg/dL', collected_at: '2026-05-01' },
      { id: 'fasting_insulin', value: 13.4, unit: 'uIU/mL', collected_at: '2026-05-01' },
    ]);
    assert.strictEqual(result.lab_data?.length, 2);
    assert.ok(result.lab_data?.every(item => item.source_type === 'measured'));
    assert.ok(result.derived_biomarkers?.some(item => item.id === 'homa_ir' && item.inputs.includes('fasting_glucose')));
    assert.ok(result.findings.some(item => item.id === 'homa_ir' && item.source_type === 'derived'));
  });

  it('should score Superpower-style qualitative urinalysis results', () => {
    const result = analyzeBiomarkers([
      { id: 'Bacteria (Urine)', value: 1, unit: 'qualitative', raw_value: 'Present' },
      { id: 'Nitrite (Urine)', value: 0, unit: 'qualitative', raw_value: 'Negative' },
      { id: 'Specific Gravity (Urine)', value: 1.032, unit: 'specific gravity' },
      { id: 'PSA % Free', value: 18, unit: '%' },
      { id: 'Z Score', value: -1.4, unit: 'z-score' },
    ]);
    assert.ok(result.findings.some(f => f.id === 'urine_bacteria' && f.status === 'needs_attention' && f.display_value === 'Present'));
    assert.ok(result.findings.some(f => f.id === 'urine_nitrite' && f.status === 'optimal'));
    assert.ok(result.findings.some(f => f.id === 'urine_specific_gravity' && f.status === 'watch'));
  ...
  it('should keep the most concerning duplicate alias finding', () => {
    ...
    const hemoglobin = result.findings.find(finding => finding.id === 'hemoglobin');
  ...
  it('should validate biomarker truthsets for statuses, derivations, and actions', () => {
    const truthsetPath = path.resolve('references/biomarker-validation-truthsets.json');
  ...
  it('should emit internal biomarker system scores and biological age estimate', () => {
    ...
    assert.ok(result.system_scores.some(score => score.id === 'cardiovascular_risk' && score.marker_count > 0));
    assert.ok(result.biological_age);
    assert.strictEqual(result.biological_age.model_version, 'biomarker-internal-v1');
  });

  it('should benchmark biomarker coverage with system-score and biological-age checks', () => {
    const requirementsPath = path.resolve('references/biomarker-benchmark-requirements.json');
    ...
    assert.ok(report.summary.system_scores >= requirements.target_system_scores);
...
describe('Whole-genome variant class engine', () => {
  it('should build a source-backed internal genetics report catalog', () => {
    ...
    assert.ok(catalog.summary.valid_entries / catalog.summary.total_entries >= 0.98);
    assert.ok(catalog.summary.source_types.length >= 6);
    assert.ok(catalog.summary.category_counts.hereditary_conditions >= 180);
    assert.ok(catalog.entries.some(entry => entry.source_type === 'acmg_secondary_findings' && entry.gene === 'BRCA1'));
    assert.ok(catalog.entries.some(entry => entry.source_type === 'recessive_carrier_gene' && entry.gene === 'CFTR'));
    assert.ok(catalog.entries.some(entry => entry.source_type === 'polygenic_risk_score'));
  });

  it('should benchmark TellmeGen-style genetics coverage for the VCF-first default path', () => {
    const requirementsPath = path.resolve('references/genetics-benchmark-requirements.json');
    const dashboardJsonPath = path.resolve('../../output/test_user_dashboard.json');
    ...
    assert.strictEqual(report.passed, true);
    assert.ok(report.summary.total_reports >= requirements.target_total_reports);
    assert.ok(report.summary.category_recall >= requirements.target_category_recall);
    ...
    assert.ok(report.metrics.some(metric => metric.id === 'external_validation' && metric.passed));
    assert.ok(report.metrics.some(metric => metric.id === 'caller_availability' && metric.passed));
    assert.ok(report.metrics.some(metric => metric.id === 'interpretation_depth_score' && metric.passed));
    assert.ok(report.metrics.some(metric => metric.id === 'interpretation_large_db_policy' && metric.passed));
    assert.strictEqual(report.summary.interpretation_requires_large_database, false);
  });

  it('should build a compact VCF-first interpretation catalog without requiring raw-read callers', () => {
    const catalog = buildCompactInterpretationCatalog({ packageDir: process.cwd() });
    assert.strictEqual(catalog.summary.raw_read_callers_required_for_default, false);
    assert.strictEqual(catalog.summary.within_repo_size_budget, true);
    assert.ok(catalog.input_model.expected_inputs.some(input => input.includes('VCF')));
    assert.ok(catalog.input_model.expected_inputs.some(input => /23andMe/i.test(input)));
    ...
    assert.ok(catalog.optional_external_sources.some(source => source.id === 'pgs_catalog'));
    assert.ok(catalog.entries.some(entry => entry.source_type === 'polygenic_score' && entry.trait === 'vo2max'));
    assert.ok(catalog.entries.some(entry => entry.source_type === 'polygenic_score' && entry.trait === 'vitamin_d'));
    assert.ok(catalog.entries.some(entry => entry.source_type === 'clinvar_gene_slice' && entry.gene === 'BRCA1'));
    assert.ok(catalog.entries.some(entry => entry.source_type === 'cpic_drug_gene_rule' && entry.gene === 'CYP2C19'));
    assert.ok(catalog.entries.some(entry => entry.source_type === 'cnv_sv_repeat_catalog' && entry.requires_deeper_caller));
  });
  ...
  it('should quantify local interpretation depth without requiring large databases', () => {
    ...
    assert.ok(report.source_families.some(source => source.id === 'clinvar_gene_slice'));
    assert.ok(report.source_families.every(source => source.large_database_required_for_default === false));
  });
  ...
  it('should expose user-facing WGS validation coverage by variant class', () => {
    ...
    assert.ok(coverage.some(item => item.query_status === 'missing_inputs_and_tools'));
    assert.ok(coverage.some(item => item.local_difficulty === 'hard'));
    assert.ok(coverage.some(item => item.status === 'pipeline_validated'));
    assert.ok(coverage.every(item => item.external_status === 'missing_inputs' || item.external_status === undefined));
  });
  ...
  it('should refresh WGS readiness artifacts used by dashboard generation', () => {
    ...
    assert.ok(result.generated.some(file => file.endsWith('wgs-caller-manifest.json')));
    assert.ok(result.generated.some(file => file.endsWith('wgs-external-truthset-setup.json')));
    assert.ok(result.generated.some(file => file.endsWith('wgs-external-validation-report.json')));
    assert.ok(result.generated.some(file => file.endsWith('wgs-query-readiness.json')));
    assert.ok(result.generated.some(file => file.endsWith('wgs-local-setup-plan.sh')));
    const setup = JSON.parse(fs.readFileSync(path.resolve('output/wgs-external-truthset-setup.json'), 'utf8'));
    const external = JSON.parse(fs.readFileSync(path.resolve('output/wgs-external-validation-report.json'), 'utf8'));
    const query = JSON.parse(fs.readFileSync(path.resolve('output/wgs-query-readiness.json'), 'utf8'));
    const setupScript = fs.readFileSync(path.resolve('output/wgs-local-setup-plan.sh'), 'utf8');
  ...
  it('should quantify local VCF fixture coverage across WGS-scale variant classes', async () => {
    ...
    assert.strictEqual(report.summary.present_vcfs, report.summary.configured_vcfs);
  ...
  it('should preflight external WGS truthsets without fabricating query VCFs', () => {
    const truthsetPath = path.resolve('references/wgs-validation-truthsets.json');
    const config = JSON.parse(fs.readFileSync(truthsetPath, 'utf8'));
    const report = buildWgsTruthsetSetupReport(config, truthsetPath, process.cwd());
    assert.strictEqual(report.summary.truthsets, 3);
    assert.ok(report.artifacts.some(artifact => artifact.kind === 'truth_vcf'));
    assert.ok(report.artifacts.some(artifact => artifact.kind === 'truth_vcf_index'));
    assert.ok(report.artifacts.some(artifact => artifact.kind === 'confident_regions'));
    assert.ok(report.artifacts.some(artifact => artifact.kind === 'query_vcf'));
    assert.ok(report.artifacts.some(artifact => artifact.kind === 'query_vcf_index'));
    assert.ok(report.artifacts.some(artifact => artifact.kind === 'metrics_json'));
    assert.ok(report.artifacts
      .filter(artifact => artifact.kind === 'query_vcf' || artifact.kind === 'query_vcf_index')
  ...
  it('should normalize CNV, SV, repeat, large-indel, and rare small variant calls', () => {
    const catalog = readWgsInterpretationCatalog(path.resolve('references/wgs-interpretation-catalog.json'));
    const vcf = fs.readFileSync(path.resolve('../../example-data/sample-wgs-variant-classes.vcf'), 'utf8');
    ...
    assert.ok(calls.some(call => call.genes.includes('BRCA1') && call.reportability === 'clinician_review'));
    assert.ok(calls.some(call => call.id === 'htt_cag_repeat' && call.evidence.some(item => item.includes('pathogenic'))));
  });

  it('should validate independent WGS truthset fixtures with class and reportability checks', () => {
    const truthsetPath = path.resolve('references/wgs-validation-truthsets.json');
    const catalogPath = path.resolve('references/wgs-interpretation-catalog.json');
    const config = JSON.parse(fs.readFileSync(truthsetPath, 'utf8'));
    const report = validateWgsVariantClassesFromTruthsets(config, truthsetPath, catalogPath, process.cwd());
    assert.strictEqual(report.passed, true);
    assert.ok(report.results.length >= 4);
    assert.ok(report.results.every(result => result.recall === 1 && result.precision === 1));
    assert.ok(report.results.every(result => result.incorrect_reportability.length === 0));
    assert.ok(report.external_validation_summary.configured_truthsets >= 3);
    assert.ok(report.external_validation.every(result => result.command_template.includes(result.benchmark_tool) || result.benchmark_tool === 'custom'));
  });
  ...
  it('should require real WGS caller runtime checks instead of treating java alone as GATK-SV readiness', () => {
    ...
    assert.strictEqual(manifest.default_input_model.raw_read_callers_required_for_default, false);
    assert.strictEqual(manifest.default_input_model.local_default_assessment, 'repo_contained_vcf_interpretation');
    assert.ok(manifest.default_input_model.expected_inputs.some(input => input.includes('VCF')));
    const haplotypeCaller = manifest.caller_steps.find(step => step.id === 'gatk_haplotypecaller_small_variants');
    const gatkSv = manifest.caller_steps.find(step => step.id === 'gatk_sv_ensemble');
  ...
  it('should quantify local query-generation readiness without creating query VCFs', () => {
    const truthsetPath = path.resolve('references/wgs-validation-truthsets.json');
    const config = JSON.parse(fs.readFileSync(truthsetPath, 'utf8'));
    const report = buildWgsQueryReadinessReport(config, truthsetPath, process.cwd());
    assert.strictEqual(report.summary.truthsets, 3);
    ...
    assert.ok(report.setup_plan.native_tool_install_hints.some(hint => hint.includes('GATK-SV') && hint.includes('Cromwell') && hint.includes('WDL')));
    assert.ok(report.setup_plan.query_generation_commands.length >= 3);
    assert.ok(report.setup_plan.postprocess_commands.some(command => command.includes('bcftools index')));
    assert.ok(report.setup_plan.validation_commands.some(command => command.includes('wgs:external-validation')));
    assert.ok(report.setup_plan.next_commands.length >= report.summary.setup_plan_steps);
    ...
    assert.ok(report.local_run_assessment.blockers.some(blocker => blocker.includes('HG002 BAM/CRAM')));
    ...
    assert.ok(report.summary.caller_container_images_present <= report.summary.caller_container_plans);
    assert.strictEqual(typeof report.summary.caller_container_image_timeouts, 'number');
    assert.ok(report.summary.benchmark_container_plans >= report.summary.benchmark_tools_required);
    assert.ok(report.summary.benchmark_container_images_present <= report.summary.benchmark_container_plans);
    assert.strictEqual(typeof report.summary.benchmark_container_image_timeouts, 'number');
    assert.strictEqual(typeof report.summary.container_runtime_available, 'boolean');
    assert.ok(report.summary.missing_inputs >= 1);
    assert.ok(report.results.every(result => result.query_vcf_path.endsWith('.vcf.gz')));
    assert.ok(report.results.every(result => result.status === 'missing_inputs_and_tools' || result.status === 'missing_inputs' || result.status === 'missing_caller_tools'));
    assert.ok(report.results.every(result => result.run_plan.setup_commands.length > 0));
    assert.ok(report.results.every(result => result.run_plan.required_tools.some(tool => tool.role === 'postprocess' && tool.tool === 'bcftools')));
    assert.ok(report.results.every(result => result.run_plan.required_tools.some(tool => tool.role === 'benchmark')));
    assert.ok(report.results.some(result => result.run_plan.required_tools.some(tool => tool.role === 'caller' && tool.container_image)));
    assert.ok(report.results.every(result => result.required_callers
      .filter(caller => caller.container_image)
      .every(caller => caller.container_pull_command?.startsWith('docker pull --platform'))));
    assert.ok(report.results.every(result => result.run_plan.required_tools
      .filter(tool => tool.role === 'benchmark' && tool.container_image)
      .every(tool => tool.container_pull_command?.startsWith('docker pull --platform'))));
    assert.ok(report.results.every(result => result.run_plan.required_tools
      .filter(tool => tool.role === 'caller' && tool.container_image)
      .every(tool => ['present', 'missing', 'timeout', 'docker_unavailable'].includes(String(tool.container_image_status)))));
    assert.ok(report.results.every(result => result.run_plan.required_tools
      .filter(tool => tool.role === 'benchmark' && tool.container_image)
      .every(tool => ['present', 'missing', 'timeout', 'docker_unavailable'].includes(String(tool.container_image_status)))));
    assert.ok(report.results.every(result => result.run_plan.caller_commands.length > 0));
    assert.ok(report.results.every(result => result.run_plan.postprocess_commands.some(command => command.includes('bcftools index'))));
    assert.ok(report.results.every(result => result.run_plan.validation_command.includes('wgs:external-validation')));
    assert.ok(report.results.every(result => result.run_plan.container_validation_command?.includes('-v "$(cd ../.. && pwd)":/work')));
    assert.ok(report.results.every(result => result.run_plan.container_validation_command?.includes('-w /work/skills/longevity-analysis')));
    assert.ok(report.results.some(result => result.run_plan.container_validation_command?.includes("giab-hg002-sv-truvari-summary")));
    assert.ok(report.results.some(result => result.run_plan.container_validation_command?.includes("giab-hg002-repeat-truvari-summary")));
    assert.ok(report.results.some(result => result.run_plan.caller_commands.some(command => command.includes('"${HG002_BAM_OR_CRAM}"'))));
    assert.ok(report.results.some(result => result.missing_inputs.includes('HG002 BAM/CRAM')));
    assert.ok(report.next_actions.some(action => /HG002 BAM\/CRAM|caller tool/i.test(action)));
  ...
  it('should preflight external WGS benchmarks and fail closed when GIAB inputs are absent', () => {
    const truthsetPath = path.resolve('references/wgs-validation-truthsets.json');
    const config = JSON.parse(fs.readFileSync(truthsetPath, 'utf8'));
    const report = buildExternalBenchmarkReport(config, truthsetPath, process.cwd());
    assert.strictEqual(report.passed, false);
    assert.ok(report.summary.configured_truthsets >= 3);
    assert.strictEqual(report.summary.passing_truthsets, 0);
    assert.ok(report.summary.missing_inputs >= 1);
    assert.ok(report.results.some(result => result.status === 'missing_inputs'));
  });
});
...
describe('Wearable Engine', () => {
  it('should score wearable behavior readings independently by domain', () => {
    ...
    assert.ok(result.score > 0 && result.score < 80);
    assert.ok(result.findings.some(f => f.id === 'sleep_duration' && f.status === 'needs_attention'));
    ...
    assert.ok(result.domains.some(d => d.id === 'sleep_recovery' && d.measured > 0));
    assert.ok(result.action_items.some(a => a.source_modalities.includes('wearables')));
  });
...
describe('Multi-modal Fusion Engine', () => {
  it('should choose biomarkers after genomics-only start', () => {
    const plan = buildMultiModalPlan({
      genomics: { connected: true, isWGS: true, trait_count: 56, top_focus_areas: ['methylation'], action_count: 4 },
      biomarkers: analyzeBiomarkers([]),
      wearables: analyzeWearables([]),
    });
    assert.strictEqual(plan.next_best_upload, 'biomarkers');
    assert.ok(plan.modalities.some(m => m.id === 'biomarkers' && m.status === 'recommended_next'));
    assert.ok(plan.action_priorities?.some(a => a.source_modalities.includes('genomics')));
  });
  ...
  it('should create cross-modal actions when labs and recovery are both off target', () => {
    ...
    const wearables = analyzeWearables([{ id: 'sleep_duration', value: 5.5 }, { id: 'hrv', value: 20 }]);
    const plan = buildMultiModalPlan({
      genomics: { connected: true, isWGS: true, trait_count: 56, top_focus_areas: ['inflammation'], action_count: 4 },
      biomarkers,
      wearables,
    });
    assert.ok(plan.action_priorities?.some(a => a.source_modalities.includes('biomarkers') && a.source_modalities.includes('wearables')));
  });
});
...
`);
    ...
    assert.ok(readings.some(r => r.id === 'apob' && r.value === 118 && r.unit === 'mg/dL'));
    assert.ok(readings.some(r => r.id === 'hba1c' && r.value === 5.7 && r.unit === '%'));
...
`);
    assert.strictEqual(readings.length, 2);
    assert.ok(readings.some(r => r.id === 'Bacteria (Urine)' && r.value === 1 && r.raw_value === 'Present'));
    assert.ok(readings.some(r => r.id === 'Nitrite (Urine)' && r.value === 0 && r.raw_value === 'Negative'));
  });

  it('should aggregate WHOOP-style daily wearable CSV files', () => {
    const readings = parseWearableCsv(`date,sleep_duration,hrv,resting_heart_rate,zone2_minutes,strength_sessions
2026-04-24,6,30,66,20,0
2026-04-25,7,40,60,30,1
`);
    assert.ok(readings.some(r => r.id === 'sleep_duration' && r.value === 6.5 && r.window_days === 2));
    assert.ok(readings.some(r => r.id === 'zone2_minutes' && r.value === 50));
    assert.ok(readings.some(r => r.id === 'strength_sessions' && r.value === 1));
  });

  it('should parse WHOOP API-shaped JSON exports', () => {
    const readings = parseWearableJson(JSON.stringify({
      window_days: 2,
      recoveries: [
        { score_state: 'SCORED', score: { recovery_score: 40, hrv_rmssd_milli: 30, resting_heart_rate: 68, spo2_percentage: 95 } },
        { score_state: 'SCORED', score: { recovery_score: 60, hrv_rmssd_milli: 40, resting_heart_rate: 62, spo2_percentage: 97 } },
      ],
      sleeps: [
        { score_state: 'SCORED', score: { sleep_efficiency_percentage: 82, respiratory_rate: 16, stage_summary: { total_in_bed_time_milli: 28800000, total_awake_time_milli: 3600000, total_slow_wave_sleep_time_milli: 3600000, total_rem_sleep_time_milli: 5400000 } } },
      ],
      cycles: [
      ...
      workouts: [
        { score_state: 'SCORED', score: { zone_duration: { zone_two_milli: 1800000, zone_four_milli: 600000, zone_five_milli: 300000 } } },
      ],
      daily_activity: [{ steps: 6000 }, { steps: 8000 }],
      manual_context: { strength_sessions: 1, sleep_consistency: 70 },
    }));
    assert.ok(readings.some(r => r.id === 'recovery_score' && r.value === 50));
    ...
    assert.ok(readings.some(r => r.id === 'vigorous_minutes' && r.value === 15));
...
describe('Priority Engine', () => {
  ...
  it('should give higher priority to below-optimal traits', () => {
    ...
    const priorities = computeAllPriorities(traits);
    const methylationRank = priorities.findIndex(p => p.trait_id === 'methylation');
    const vitaminDRank = priorities.findIndex(p => p.trait_id === 'vitamin_d');
    assert.ok(methylationRank < vitaminDRank, 'Low-scoring trait should rank higher priority');
  });
...
describe('Protocol Engine', () => {
  ...
  it('should include Priority Wellness Action Plan for at-risk traits', () => {
    ...
    const protocols = generateProtocols(traits);
    const core = protocols.find(p => p.title === 'Priority Wellness Action Plan');
  ...
  it('should deduplicate actions across traits', () => {
    ...
    assert.strictEqual(allActionIds.length, uniqueIds.size, 'All action IDs should be unique');
  });
...
describe('New Longevity Traits', () => {
  it('should enrich caloric_restriction trait', () => {
    ...
    assert.ok(traits[0].actions!.some(a => a.id === 'time_restricted_eating'));
    assert.ok(traits[0].actions!.some(a => a.id === 'avoid_extreme_cr'));
  });

  it('should enrich intermittent_fasting trait', () => {
    const traits = enrichTraits([
      { trait_id: 'intermittent_fasting', score: 60, confidence: 0.70 }
    ]);
    assert.ok(traits[0].actions);
  ...
  it('should enrich apoB_management trait', () => {
    const traits = enrichTraits([
      { trait_id: 'apoB_management', score: 75, confidence: 0.90 }
    ]);
    assert.ok(traits[0].actions);
  ...
  it('should enrich immune_rejuvenation trait', () => {
    const traits = enrichTraits([
      { trait_id: 'immune_rejuvenation', score: 60, confidence: 0.60 }
    ]);
    assert.ok(traits[0].actions);
  ...
  it('should enrich partial_reprogramming trait', () => {
    const traits = enrichTraits([
      { trait_id: 'partial_reprogramming', score: 40, confidence: 0.50 }
    ]);
  ...
  it('should enrich stem_cells trait with safety warnings', () => {
    const traits = enrichTraits([
      { trait_id: 'stem_cells', score: 50, confidence: 0.60 }
    ]);
  ...
  it('should enrich supplements_evidence with evidence-based guidance', () => {
    const traits = enrichTraits([
      { trait_id: 'supplements_evidence', score: 70, confidence: 0.90 }
    ]);
    assert.ok(traits[0].actions!.some(a => a.id === 'avoid_nmn_alone'));
    assert.ok(traits[0].actions!.some(a => a.id === 'vitamin_d_only_if_deficient'));
  });

  it('should link trametinib_rapamycin to safety concerns', () => {
    const traits = enrichTraits([
      { trait_id: 'trametinib_rapamycin', score: 45, confidence: 0.50 }
    ]);
...
describe('Hallmark Longevity Traits', () => {
  it('should enrich protein_homeostasis with chaperone actions', () => {
    const traits = enrichTraits([
      { trait_id: 'protein_homeostasis', score: 65, confidence: 0.80 }
    ]);
    assert.ok(traits[0].actions);
  ...
  it('should enrich proteasome_autophagy trait', () => {
    const traits = enrichTraits([
      { trait_id: 'proteasome_autophagy', score: 65, confidence: 0.80 }
    ]);
    assert.ok(traits[0].actions!.some(a => a.id === 'intermittent_fasting_autophagy'));
  ...
  it('should enrich genome_stability with cancer screening action', () => {
    ...
    assert.ok(traits[0].outcomes!.some(o => o.id === 'dna_damage_accumulation'));
  ...
  it('should enrich mTOR_signaling with rapamycin and protein cycling', () => {
    const traits = enrichTraits([
      { trait_id: 'mTOR_signaling', score: 60, confidence: 0.80 }
    ]);
  ...
  it('should enrich epigenetic_maintenance trait', () => {
    const traits = enrichTraits([
      { trait_id: 'epigenetic_maintenance', score: 60, confidence: 0.70 }
    ]);
...
describe('Evidence Weighting', () => {
  ...
  it('should compute evidence-weighted GLI', () => {
    const traits = enrichTraits([
      ...
      { trait_id: 'caffeine_metabolism', score: 30, confidence: 0.85, evidenceTier: 3 },
    ]);
    ...
    assert.ok(gli > unweighted, `Evidence-weighted (${gli}) should be > unweighted (${unweighted}) when high-tier is high-score`);
  });
  ...
  it('should give higher evidence-weighted GLI when Tier 1 traits score high', () => {
    const traits = enrichTraits([
      ...
      { trait_id: 'caffeine_metabolism', score: 10, confidence: 0.85, evidenceTier: 3 },
    ]);
    const gli = computeEvidenceWeightedGLI(traits);
    // (90*1.0 + 10*0.4) / (1.0 + 0.4) = (90 + 4) / 1.4 = 67.14 → *10 = 671
    const unweighted = computeGLI(traits);
    assert.strictEqual(unweighted, 500); // (90+10)/2 * 10
    assert.ok(gli > unweighted, `Evidence-weighted (${gli}) should be > unweighted (${unweighted})`);
  });
...
describe('ClinVar ACMG & Rare Variants', () => {
  ...
  it('should have ACMG info for BRCA1', () => {
    ...
    assert.ok(ACMG_GENE_INFO['BRCA1'].recommendation.includes('genetic counselor'));
  });

  it('should have ACMG info for RYR1 (malignant hyperthermia)', () => {
    assert.ok(ACMG_GENE_INFO['RYR1']);
    assert.ok(ACMG_GENE_INFO['RYR1'].condition.includes('Malignant Hyperthermia'));
  ...
  it('should include the full 71-gene hereditary multi-cancer panel separately from ACMG', () => {
    ...
    for (const gene of ['BRCA1', 'BRCA2', 'ATM', 'MLH1', 'MSH2', 'TP53', 'APC', 'PTEN', 'RET', 'VHL', 'MEN1']) {
      assert.ok(HEREDITARY_CANCER_PANEL_GENES.includes(gene as any), `${gene} should be covered`);
    }
...
describe('Edge Cases', () => {
  it('should handle score 0', () => {
    const traits = enrichTraits([{ trait_id: 'methylation', score: 0, confidence: 0.9 }]);
    const gli = computeGLI(traits);
    assert.strictEqual(gli, 0);
    const insights = generateInsights(traits);
  ...
  it('should handle score 100', () => {
    const traits = enrichTraits([{ trait_id: 'cardiovascular_fitness', score: 100, confidence: 1.0 }]);
    const gli = computeGLI(traits);
...
describe('VEP Annotation', () => {
  it('should parse VEP tabular output', () => {
    ...
    try {
      ...
      assert.ok(result.size > 0, `Expected > 0 annotations, got ${result.size}`);
  ...
  it('should provide on-demand VEP lookup via grep', () => {
    ...
    try {
      ...
      assert.ok(synResult, 'Should find LOW impact variant via on-demand lookup');
      ...
      assert.ok(!result.has(synVariant), 'LOW impact should NOT be in the in-memory set');
    } finally {
      try { fs.unlinkSync(tmpFile); } catch (_) {}
    }
  });
  ...
  it('should batch-lookup multiple VEP variants', () => {
    ...
    try {
      const results = queryVEPForVariants(['1:11856378:G:A', '7:117559590:G:A'], tmpFile);
