...
function qualitativeCode(value: string | undefined): number | undefined {
  ...
  if (/(positive|present|detected|abnormal|moderate|many|large|cloudy|turbid|red|brown)/.test(normalized)) return 1;
...
function hoursFromMillis(value: unknown): number | undefined {
  ...
  return millis == null ? undefined : Math.round((millis / 3_600_000) * 100) / 100;
}
...
export function parseBiomarkerText(text: string): BiomarkerReading[] {
  ...
  const collectedAt = text.match(/Collected:\s*([0-9]{4}-[0-9]{2}-[0-9]{2})/i)?.[1];
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    for (const pattern of BIOMARKER_PATTERNS) {
      ...
      for (const label of pattern.labels) {
        const re = new RegExp(`\\b${escapeRegex(label)}\\b\\s*[:\\-]?\\s*(-?\\d+(?:\\.\\d+)?)\\s*([^,;\\n]*)`, 'i');
        const match = trimmed.match(re);
        if (!match) continue;
        const value = numeric(match[1]);
        if (value == null) continue;
        const unit = match[2]?.trim() || undefined;
        readings.push({ id: pattern.id, value, unit, collected_at: collectedAt });
        seen.add(pattern.id);
        break;
      }
      if (seen.has(pattern.id)) break;
      for (const label of pattern.labels) {
        const re = new RegExp(`\\b${escapeRegex(label)}\\b\\s*[:\\-]?\\s*([A-Za-z][A-Za-z0-9 +/\\-]*)`, 'i');
        const match = trimmed.match(re);
        if (!match) continue;
        const rawValue = match[1]?.trim();
        const value = qualitativeCode(rawValue);
        if (value == null || !rawValue) continue;
        readings.push({ id: pattern.id, value, unit: 'qualitative', raw_value: rawValue, collected_at: collectedAt });
        seen.add(pattern.id);
        break;
      }
      if (seen.has(pattern.id)) break;
...
export function parseBiomarkerCsv(text: string): BiomarkerReading[] {
  return parseCsv(text)
    .map((row): BiomarkerReading | undefined => {
      const id = firstDefined(row, ['marker', 'biomarker', 'test', 'test_name', 'name', 'id']);
      ...
      return {
        id,
        value,
        unit: firstDefined(row, ['unit', 'units']) ?? (numberFrom(rawValue) == null ? 'qualitative' : undefined),
        raw_value: rawValue,
        collected_at: firstDefined(row, ['collected_at', 'date', 'result_date']),
      };
    })
...
export function parseBiomarkerJson(text: string): BiomarkerReading[] {
  ...
  const rows = Array.isArray(parsed)
    ? parsed
    : arrayFrom(objectFrom(parsed).biomarkers ?? objectFrom(parsed).results ?? objectFrom(parsed).markers);

  return rows
    .map((row): BiomarkerReading | undefined => {
      if (row == null || typeof row !== 'object' || Array.isArray(row)) return undefined;
      const item = row as JsonObject;
      const id = String(item.marker ?? item.biomarker ?? item.test_name ?? item.name ?? item.id ?? '').trim();
      const rawValue = item.value ?? item.result ?? item.result_value;
      const value = numeric(rawValue) ?? (typeof rawValue === 'string' ? qualitativeCode(rawValue) : undefined);
      if (!id || value == null) return undefined;
      return {
        id,
        value,
        unit: typeof item.unit === 'string' ? item.unit : typeof item.units === 'string' ? item.units : typeof rawValue === 'string' && numeric(rawValue) == null ? 'qualitative' : undefined,
        raw_value: typeof rawValue === 'string' ? rawValue : undefined,
        collected_at: typeof item.collected_at === 'string' ? item.collected_at : typeof item.date === 'string' ? item.date : undefined,
      };
    })
    .filter((reading): reading is BiomarkerReading => reading != null);
}
...
export function parseBiomarkerFile(filePath: string): BiomarkerReading[] {
  ...
  if (lower.endsWith('.txt') || lower.endsWith('.md')) return parseBiomarkerText(text);
...
export function parseWearableCsv(text: string): WearableReading[] {
  ...
  for (const row of rows) {
    for (const [column, rawValue] of Object.entries(row)) {
      ...
      const aggregate = aggregates.get(id) ?? { values: [], unit: WEARABLE_UNITS[id] };
      aggregate.values.push(value);
      aggregates.set(id, aggregate);
    }
  }

  return Array.from(aggregates.entries()).map(([id, aggregate]) => {
...
function pushReading(readings: WearableReading[], id: string, value: number | undefined, unit?: string, windowDays?: number): void {
  if (value == null || !Number.isFinite(value)) return;
  readings.push({ id, value, unit: unit ?? WEARABLE_UNITS[id], window_days: windowDays });
}
...
export function parseWearableJson(text: string): WearableReading[] {
  ...
  const inferredWindowDays = Math.max(recoveries.length, sleeps.length, cycles.length, dailyActivity.length);
  const windowDays = numeric(root.window_days) ?? (inferredWindowDays > 0 ? inferredWindowDays : undefined);
  ...
  pushReading(readings, 'recovery_score', average(recoveryScores), '%', windowDays);
  ...
  pushReading(readings, 'skin_temperature', average(skinTemperature), 'C', windowDays);
  ...
  for (const sleep of sleeps) {
    ...
    const sleepHours = inBed == null ? undefined : Math.max(0, Math.round((inBed - awake) * 100) / 100);
  ...
  pushReading(readings, 'sleep_duration', average(sleepDuration), 'hours', windowDays);
  pushReading(readings, 'sleep_efficiency', average(sleepEfficiency), '%', windowDays);
  pushReading(readings, 'sleep_performance', average(sleepPerformance), '%', windowDays);
  pushReading(readings, 'deep_sleep', average(deepSleep), 'hours', windowDays);
  pushReading(readings, 'light_sleep', average(lightSleep), 'hours', windowDays);
  pushReading(readings, 'rem_sleep', average(remSleep), 'hours', windowDays);
  pushReading(readings, 'respiratory_rate', average(respiratoryRate), 'rpm', windowDays);
  ...
  pushReading(readings, 'average_heart_rate', average(averageHeartRateValues), 'bpm', windowDays);
  pushReading(readings, 'max_heart_rate', average(maxHeartRateValues), 'bpm', windowDays);
  ...
  pushReading(readings, 'zone2_minutes', sum(zone2Minutes), 'min/week', windowDays);
  pushReading(readings, 'vigorous_minutes', sum(vigorousMinutes), 'min/week', windowDays);
  pushReading(readings, 'workout_count', workouts.length > 0 ? workouts.length : undefined, 'sessions/week', windowDays);

  const steps = dailyActivity.map(day => numeric(day.steps)).filter((value): value is number => value != null);
  ...
  for (const [key, rawValue] of Object.entries(manual)) {
    ...
    pushReading(readings, id, numeric(rawValue), WEARABLE_UNITS[id], windowDays);
  }
...
export function parseWearableFile(filePath: string): WearableReading[] {
  const text = fs.readFileSync(filePath, 'utf8');
  return filePath.toLowerCase().endsWith('.json') ? parseWearableJson(text) : parseWearableCsv(text);
}
