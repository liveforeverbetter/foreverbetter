...
import { renderFullDashboard } from '../../shared/design/render-designs.js';
import { PipelineLogger } from './pipeline_logger.js';

import { analyzeVCF } from '../ingestion/parse-vcf.js';
import type { LongevityProtocol, AnalyzeVCFResult } from '../ingestion/parse-vcf.js';
import { enrichTraits } from './graph_resolver.js';
import type { EnrichedTrait } from './graph_resolver.js';
import { queryClinVarForRSIDs, generateVariantAnnotation, categorizeVariantForTab, isRecessiveDiseaseGene, type ClinVarAnnotation, type VariantCategory, type ClinVarConfidenceTier } from './clinvar_enrichment.js';
...
import { computeWeightedGLI, getGLIRating, computeWeightedCategoryGLI } from './gli_engine.js';
import { computeHallmarkScores, type HallmarkReport } from './hallmark_engine.js';
...
import { enrichVEPMissenseLongevity, mapVEPMissenseToTraits, type MissenseCall } from './vep_missense_enrichment.js';
...
import type { PersonalizedActionPlan, PersonalizedAction, PlanReviewItem, PlanModality, PlanSafetyTier } from '../../shared/dashboard-types.js';
...
import { buildWgsQueryReadinessReport, renderWgsLocalSetupScript } from './wgs_query_readiness.js';
import { runVcfDoctor } from './vcf_doctor.js';
import { renderDashboard } from '../../src/renderer/render.js';
import { surfaceAcrossModalities, type CatalogMatchSummary } from './catalog_loader.js';
import { resolveCatalogEvidence, type CatalogFindings } from './catalog_evidence_resolver.js';
import type {
  DashboardData, Meta, Gli, InnateStrength,
  Category, Insight as RInsight, ActionItem,
  Protocol as RProtocol, ProtocolPhase,
  StatusColor, ActionPriority,
  ClinVarVariantCard, GeneticVariantsSection,
  SignificanceColor, PRSScore as DashboardPRSScore,
  VEPMissenseSection, WgsValidationCoverage,
...
export { computeWeightedGLI, computeWeightedCategoryGLI } from './gli_engine.js';
...
export interface DashboardOutput {
  ...
  top_traits: Array<{ trait_id: string; score: number; confidence: number; mechanism: string }>;
...
interface WgsTruthsetSetupReportFile {
  artifacts?: Array<{
    truthset_id?: string;
    kind?: 'truth_vcf' | 'truth_vcf_index' | 'confident_regions' | 'query_vcf' | 'query_vcf_index' | 'metrics_json';
    present?: boolean;
...
function readLocalVcfCoverage(packageDir?: string): LocalVcfCoverageSummary | undefined {
  const scriptDir = path.dirname(fileURLToPath(import.meta.url));
  const resolvedPackageDir = packageDir ?? path.resolve(scriptDir, '../..');
  return readOptionalJsonFile<LocalVcfCoverageReportFile>(path.join(resolvedPackageDir, 'output/local-vcf-coverage.json'))?.summary;
}
...
export function refreshWgsReadinessArtifacts(packageDir?: string): WgsReadinessRefreshResult {
  const scriptDir = path.dirname(fileURLToPath(import.meta.url));
  const resolvedPackageDir = packageDir ?? path.resolve(scriptDir, '../..');
  const outputDir = path.join(resolvedPackageDir, 'output');
  const truthsetPath = path.join(resolvedPackageDir, 'references/wgs-validation-truthsets.json');
  ...
  try {
    ...
    fs.writeFileSync(callerManifestPath, `${JSON.stringify(callerManifest, null, 2)}\n`, 'utf8');
    generated.push(callerManifestPath);
  } catch (error) {
    errors.push(`caller manifest: ${error instanceof Error ? error.message : String(error)}`);
  }

  const truthsetConfig = readOptionalJsonFile<any>(truthsetPath);
  if (!truthsetConfig) {
    skipped.push(`External truthset config missing or unreadable: ${truthsetPath}`);
    return { generated, skipped, errors };
  }

  try {
    const setupPath = path.join(outputDir, 'wgs-external-truthset-setup.json');
    const setupReport = buildWgsTruthsetSetupReport(truthsetConfig, truthsetPath, resolvedPackageDir);
    fs.writeFileSync(setupPath, `${JSON.stringify(setupReport, null, 2)}\n`, 'utf8');
    generated.push(setupPath);
  } catch (error) {
    errors.push(`truthset setup: ${error instanceof Error ? error.message : String(error)}`);
  }

  try {
    const externalPath = path.join(outputDir, 'wgs-external-validation-report.json');
    const externalReport = buildExternalBenchmarkReport(truthsetConfig, truthsetPath, resolvedPackageDir, false);
    fs.writeFileSync(externalPath, `${JSON.stringify(externalReport, null, 2)}\n`, 'utf8');
    generated.push(externalPath);
  } catch (error) {
    errors.push(`external validation preflight: ${error instanceof Error ? error.message : String(error)}`);
  }

  try {
    const queryReadinessPath = path.join(outputDir, 'wgs-query-readiness.json');
    const querySetupScriptPath = path.join(outputDir, 'wgs-local-setup-plan.sh');
    const queryReadiness = buildWgsQueryReadinessReport(truthsetConfig, truthsetPath, resolvedPackageDir);
    queryReadiness.setup_script_path = querySetupScriptPath;
    fs.writeFileSync(queryReadinessPath, `${JSON.stringify(queryReadiness, null, 2)}\n`, 'utf8');
    generated.push(queryReadinessPath);
    fs.writeFileSync(querySetupScriptPath, renderWgsLocalSetupScript(queryReadiness), 'utf8');
    fs.chmodSync(querySetupScriptPath, 0o755);
    generated.push(querySetupScriptPath);
  } catch (error) {
    errors.push(`query generation readiness: ${error instanceof Error ? error.message : String(error)}`);
  }

  return { generated, skipped, errors };
}

export function buildWgsValidationCoverage(packageDir?: string): WgsValidationCoverage[] {
  const scriptDir = path.dirname(fileURLToPath(import.meta.url));
  const resolvedPackageDir = packageDir ?? path.resolve(scriptDir, '../..');
  const outputDir = path.join(resolvedPackageDir, 'output');
  const classSummary = readOptionalJsonFile<WgsVariantClassSummaryFile>(path.join(outputDir, 'wgs-variant-class-summary.json'));
  const validation = readOptionalJsonFile<WgsValidationReportFile>(path.join(outputDir, 'wgs-validation-report.json'));
  const external = readOptionalJsonFile<WgsExternalValidationReportFile>(path.join(outputDir, 'wgs-external-validation-report.json'));
  const setup = readOptionalJsonFile<WgsTruthsetSetupReportFile>(path.join(outputDir, 'wgs-external-truthset-setup.json'));
  const queryReadiness = readOptionalJsonFile<WgsQueryReadinessReportFile>(path.join(outputDir, 'wgs-query-readiness.json'));
  const manifest = readOptionalJsonFile<WgsCallerManifestFile>(path.join(outputDir, 'wgs-caller-manifest.json'));

  return Object.entries(WGS_VARIANT_CLASS_LABELS).map(([id, label]) => {
    const callCount = classSummary?.class_counts?.[id] ?? 0;
    const reportableCount = (classSummary?.calls ?? []).filter(call =>
      call.class === id && call.reportability && call.reportability !== 'research_only'
    ).length;
    const syntheticResults = (validation?.results ?? []).filter(result =>
      (result.expected_classes ?? []).includes(id) || (result.observed_classes ?? []).includes(id)
    );
    const syntheticValidated = syntheticResults.some(result => result.passed && !(result.missing_classes ?? []).includes(id));
    const bestSynthetic = syntheticResults
      .filter(result => result.recall != null && result.precision != null)
      .sort((a, b) => ((b.recall ?? 0) + (b.precision ?? 0)) - ((a.recall ?? 0) + (a.precision ?? 0)))[0];
    const externalResults = (external?.results ?? []).filter(result => (result.variant_classes ?? []).includes(id));
    ...
    const queryResults = (queryReadiness?.results ?? []).filter(result => (result.variant_classes ?? []).includes(id));
    const mostRelevantQuery = queryResults.find(result => result.status === 'ready_to_validate')
      ?? queryResults.find(result => result.ready_to_generate)
      ?? queryResults[0];
    const callerSteps = (manifest?.caller_steps ?? []).filter(step =>
      (step.variant_class ?? '').split(',').map(item => item.trim()).includes(id)
    );
    const callerTools = callerSteps.map(step => step.tool).filter((tool): tool is string => Boolean(tool));
    const callerAvailable = callerSteps.some(step => step.available);
    const setupArtifacts = (setup?.artifacts ?? []).filter(artifact =>
      externalResults.some(result => result.truthset_id === artifact.truthset_id)
    );
    const missingTruthArtifacts = setupArtifacts.filter(artifact =>
      (artifact.kind === 'truth_vcf' || artifact.kind === 'truth_vcf_index' || artifact.kind === 'confident_regions') && !artifact.present
    );
    const missingQueryArtifacts = setupArtifacts.filter(artifact =>
      (artifact.kind === 'query_vcf' || artifact.kind === 'query_vcf_index') && !artifact.present
    );
    const missingMetricsArtifacts = setupArtifacts.filter(artifact => artifact.kind === 'metrics_json' && !artifact.present);
    const externalTools = new Set(externalResults.map(result => result.benchmark_tool).filter((tool): tool is string => Boolean(tool)));
    ...
    const note = passingExternal
      ? `External benchmark passed against ${passingExternal.source_name ?? passingExternal.truthset_id}.`
      : syntheticValidated
        ? `Parser and interpretation fixture passed (${formatPercentMetric(bestSynthetic?.recall)} recall, ${formatPercentMetric(bestSynthetic?.precision)} precision); GIAB-style external validation is still pending.`
        : externalResults.length > 0
          ? `External benchmark configured but not yet passing (${mostRelevantExternal?.status ?? 'pending'}).`
    ...
    const nextStep = passingExternal
      ...
      : missingTruthArtifacts.length > 0
        ...
        : missingQueryArtifacts.length > 0
          ? `Next: ${mostRelevantQuery?.next_action ?? 'run the matching HG002 caller pipeline to generate the local query VCF.'}`
          : missingTools.length > 0
            ? `Next: install the benchmark tool ${missingTools.map(tool => tool.tool).filter(Boolean).join(' / ')}.`
            : missingMetricsArtifacts.length > 0
              ? 'Next: run external validation to produce precision/recall metrics.'
              : externalResults.length > 0
                ? `Next: resolve external validation status ${mostRelevantExternal?.status ?? 'pending'}.`
    ...
    return {
      ...
      external_source: mostRelevantExternal?.source_name ?? mostRelevantExternal?.truthset_id,
 ...
 */
function mapProtocolToTraits(protocol: LongevityProtocol): Array<{ trait_id: string; score: number; confidence: number; evidenceTier?: 1 | 2 | 3 }> {
  ...
  function categorizeUnknownGene(raw: string): string {
    ...
    if (/cancer|tumor|neoplas|melanoma|leukemia|lymphoma|sarcoma|carcinoma/i.test(g)) return 'hereditary_cancer';
    if (/heart|cardio|arrhythmia|long_qt|brugada|dilated|hypertrophic/i.test(g)) return 'cardiomyopathy';
    if (/alzheimer|parkinson|dementia|huntington|neurodeg|ataxia|spastic/i.test(g)) return 'neurological_health';
    if (/lupus|autoimmune|rheumatoid|multiple_sclerosis|crohn|colitis|celiac|psoriasis/i.test(g)) return 'immune_response';
    if (/eye|retin|macular|cornea|blind|vision|cataract|glaucoma/i.test(g)) return 'eye_health';
    if (/kidney|renal|nephro|polycystic/i.test(g)) return 'kidney_health';
    if (/bone|skeletal|osteop|osteogenesis|dwarfism|chondro/i.test(g)) return 'bone_health';
    if (/clot|thrombo|hemophilia|bleeding|factor_/i.test(g)) return 'thrombosis_risk';
    if (/connective|marfan|ehlers|aneurysm|aort/i.test(g)) return 'connective_tissue';
    if (/metabol|diabetes|obesity|thyroid|endocrine/i.test(g)) return 'glucose_metabolism';
    if (/deaf|hearing|ear/i.test(g)) return 'hearing_health';
    if (/muscular|muscle|myopathy|dystrophy/i.test(g)) return 'muscle_performance';
    if (/liver|hepatitis|cirrhosis|biliary/i.test(g)) return 'liver_health';
    if (/lung|pulmonary|respiratory|asthma|copd/i.test(g)) return 'respiratory_health';
  ...
  const traits: Array<{ trait_id: string; score: number; confidence: number; evidenceTier?: 1 | 2 | 3 }> = [];
  ...
  for (const sp of protocol.genomicProfile.superpowers) {
    ...
    if (sp.rsid && sp.rsid.startsWith('rs') && FAVORABLE_EDGE_CATALOG[sp.rsid]) {
      traits.push({
  ...
  for (const supp of protocol.dailyStack.morning) {
    ...
    if (reason.includes('omega') || reason.includes('inflammation') || reason.includes('anti-inflammatory')) {
      traits.push({ trait_id: 'inflammation', score: 50, confidence: 0.80 });
    }
    if (reason.includes('b12') || reason.includes('cobalamin')) {
      traits.push({ trait_id: 'b12_metabolism', score: 55, confidence: 0.85 });
    }
    if (reason.includes('lipid') || reason.includes('ldl') || reason.includes('statin')) {
      traits.push({ trait_id: 'cholesterol', score: 40, confidence: 0.85 });
    }
    if (reason.includes('coa') || reason.includes('q10') || reason.includes('mitochondrial')) {
      traits.push({ trait_id: 'mitochondrial', score: 55, confidence: 0.80 });
    }
  }

  // Deduplicate by trait_id, taking the lowest score (most concern), preserving evidence tier
  const traitMap = new Map<string, { trait_id: string; score: number; confidence: number; evidenceTier?: 1 | 2 | 3 }>();
  for (const trait of traits) {
    const existing = traitMap.get(trait.trait_id);
    if (!existing || trait.score < existing.score) {
      traitMap.set(trait.trait_id, trait);
    } else if (existing && trait.score === existing.score && trait.evidenceTier !== undefined && existing.evidenceTier === undefined) {
  ...
  const baselineLongevityTraits: Array<{ trait_id: string; score: number; confidence: number }> = [
    ...
    { trait_id: 'baseline_labs', score: 75, confidence: 0.90 }, // Annual labs
    { trait_id: 'body_composition', score: 75, confidence: 0.85 }, // Waist circumference, DEXA
    { trait_id: 'glucose_insulin_management', score: 75, confidence: 0.90 }, // Annual glucose/insulin
    { trait_id: 'lipid_management', score: 75, confidence: 0.90 }, // Annual lipid panel + apoB
    { trait_id: 'blood_pressure_control', score: 75, confidence: 0.90 }, // BP monitoring
    { trait_id: 'caloric_restriction', score: 60, confidence: 0.75 }, // Moderate CR / time-restricted eating
    { trait_id: 'intermittent_fasting', score: 60, confidence: 0.70 }, // IF protocols
    { trait_id: 'apoB_management', score: 75, confidence: 0.90 }, // apoB as superior lipid marker
    { trait_id: 'immune_rejuvenation', score: 60, confidence: 0.60 }, // Emerging immunosenescence reversal
    { trait_id: 'brain_rejuvenation', score: 65, confidence: 0.75 }, // Brain anti-aging (exercise, sleep, cognition)
    { trait_id: 'supplements_evidence', score: 70, confidence: 0.90 }, // Evidence-based supplement guidance
    { trait_id: 'protein_homeostasis', score: 65, confidence: 0.80 }, // Proteostasis via heat shock, chaperones
    { trait_id: 'senescence', score: 60, confidence: 0.75 },         // Cellular senescence / SASP burden
    { trait_id: 'proteasome_autophagy', score: 65, confidence: 0.80 }, // Autophagy and protein clearance
    { trait_id: 'genome_stability', score: 70, confidence: 0.85 },    // DNA repair capacity
    { trait_id: 'epigenetic_maintenance', score: 60, confidence: 0.70 }, // Epigenetic drift / aging clocks
    { trait_id: 'klotho_anti_aging', score: 60, confidence: 0.65 },  // Klotho levels and cognitive protection
    { trait_id: 'mTOR_signaling', score: 60, confidence: 0.80 },      // mTOR pathway and rapamycin
  ];
...
function mapClinVarToTraits(
  ...
  if (Array.isArray(clinvarAnnotations)) {
    ...
    entries = clinvarAnnotations.map((a: any) => [a.rsid || '', a] as [string, any]);
  ...
  } else if (typeof clinvarAnnotations.size === 'number' && clinvarAnnotations.size > 0) {
    // Map<string, any>
  ...
  for (const [rsid, annotation] of entries) {
    const clinicalSig = annotation?.clinical_significance || annotation?.clinicalSignificance || '';
    const diseaseName = annotation?.disease_name || annotation?.diseaseName || annotation?.trait || '';
    ...
    if (isStrictPathogenicClinVarSignificance(clinicalSig)) {
      ...
      const geneSymbol = (annotation?.gene_symbol || annotation?.gene || '').toUpperCase();
      const isRecessive = isRecessiveDiseaseGene(geneSymbol);
      const isHeterozygous = !annotation?.zygosity || annotation?.zygosity === 'Heterozygous';
      ...
      } else if (/heart|cardio|arrhythmia|aort|myopathy|brugada|long qt/i.test(diseaseName)) {
        traitId = 'cardiomyopathy_risk';
      } else if (/alzheimer|parkinson|dementia|huntington|als|neurodeg/i.test(diseaseName)) {
        traitId = 'neurodegeneration_risk';
      } else if (/lipid|cholesterol|hyperchol|ldl|apob|familial/i.test(diseaseName)) {
...
function mapCPICToTraits(
  ...
  if (!cpicResult || !cpicResult.matches || cpicResult.matches.length === 0) return [];
...
function mapVEPToTraits(
  ...
  for (const [key, ann] of vepAnnotations) {
    ...
    traits.push({
      trait_id: traitId,
      score,
      confidence: ann.impact === 'HIGH' ? 0.90 : 0.75,
    });
  }
...
function mergeTraitScores(
  existing: Array<{ trait_id: string; score: number; confidence: number; evidenceTier?: 1 | 2 | 3 }>,
  ...additional: Array<Array<{ trait_id: string; score: number; confidence: number; evidenceTier?: 1 | 2 | 3 }>>
): Array<{ trait_id: string; score: number; confidence: number; evidenceTier?: 1 | 2 | 3 }> {
  const merged = new Map<string, { trait_id: string; score: number; confidence: number; evidenceTier?: 1 | 2 | 3 }>();
  ...
  for (const batch of additional) {
    for (const trait of batch) {
      ...
      } else if (trait.score === (current?.score || Infinity) &&
                 trait.evidenceTier !== undefined && current?.evidenceTier === undefined) {
        merged.set(trait.trait_id, { ...trait, confidence: Math.max(trait.confidence, current.confidence) });
      }
    }
  }
...
function confidenceTierLabel(tier?: ClinVarConfidenceTier): string {
  switch (tier) {
    case "pathogenic_likely_pathogenic": return "Pathogenic / likely pathogenic";
    case "drug_response": return "Drug response";
    case "risk_factor_protective": return "Risk factor / protective";
    case "vus": return "Variant of uncertain significance";
    case "benign": return "Benign / likely benign";
    case "conflicting_classifications": return "Conflicting classifications";
    default: return "Other ClinVar classification";
  }
}

function countClinVarConfidenceTiers(annotations?: ClinVarAnnotation[]): Partial<Record<ClinVarConfidenceTier, number>> {
...
function buildClinVarVariantCards(
  ...
  if (Array.isArray(clinvarAnnotations)) {
    ...
    annotationEntries = clinvarAnnotations.map((a: any) => [a.rsid || '', a] as [string, any]);
  } else if (Array.isArray(clinvarAnnotations.annotations)) {
    // ClinVarEnrichmentResult with annotations array
    const list: any[] = clinvarAnnotations.annotations;
    if (list.length === 0) return cards;
    annotationEntries = list.map((a: any) => [a.rsid || '', a] as [string, any]);
  } else if (typeof clinvarAnnotations.size === 'number' && clinvarAnnotations.size > 0) {
    // Map-like object
  ...
  for (const [rsid, raw] of annotationEntries) {
    const annotation: ClinVarAnnotation = {
      rsid: raw.rsid || rsid,
      clinicalSignificance: raw.clinicalSignificance || raw.clinical_significance || "not_provided",
      diseaseName: raw.diseaseName || raw.disease_name || raw.trait || "not_specified",
    ...
    if (vepAnnotations) {
      for (const [key, vep] of vepAnnotations) {
        if ((vep.rsid || "").toString() === rsid.toString() ||
            key.includes(rsid.toString())) {
          if (vep.cadd_phred !== undefined) caddScore = vep.cadd_phred;
          if (vep.cadd_raw !== undefined && caddScore === undefined) caddScore = vep.cadd_raw;
          break;
        }
      }
    }
    ...
    const consumerAnnotation = generateVariantAnnotation(annotation, zygosity, caddScore);

    // Categorize
    const category = categorizeVariantForTab(annotation);

    const gene = annotation.geneInfo.split(":")[0] || annotation.geneInfo.split("|")[0] || "Unknown";
    const disease = annotation.diseaseName.replace(/_/g, " ").replace(/not_specified|not provided/i, "").trim() || "Not specified";
    const frequency = annotation.populationFrequency || (annotation.isRare ? "Rare (<1%)" : "Common (>5%)");
...
export async function runPipelineFromVCF(
  ...
  logDir?: string,
  options: { annotationDepth?: 'compact' | 'full_dbsnp'; dbsnpPath?: string } = {},
): Promise<DashboardOutput> {
  const startTime = Date.now();
  const errors: PipelineError[] = [];
  const steps: Array<{ step: string; start: number; duration_ms: number; status: 'ok' | 'error' }> = [];
  const markStart = (step: string) => { const s = Date.now(); steps.push({ step, start: s, duration_ms: 0, status: 'ok' }); return s; };
  const markEnd = (start: number, status: 'ok' | 'error' = 'ok') => {
    const s = steps[steps.length - 1];
    if (s) { s.duration_ms = Date.now() - start; s.status = status; }
  };
  ...
  let topTraits: Array<{ trait_id: string; score: number; confidence: number; mechanism: string }> = [];
  let hallmark: HallmarkReport = { hallmarks: [], total_genes_hit: 0, hallmarks_affected: 0, summary: '' };
  let variantCards: GeneticVariantsSection = { genetic_conditions: [], drug_response: [], other_risks: [], rare_mutations: [], uncommon_mutations: [] };
  ...
  if (wgsReadiness.errors.length > 0) {
    errors.push({ step: 'wgs_readiness_preflight', error: wgsReadiness.errors.join('; ') });
  }
  ...
  try {
    const vcfDir = path.dirname(path.resolve(vcfPath));
    ...
    // Check if pre-annotated VCF exists
    const annotatedPath1 = path.join(vcfDir, `${vcfBasename}.annotated.vcf.gz`);
    const annotatedPath2 = path.join(vcfDir, `${vcfBasename}.vcf.annotated.vcf.gz`);
    const hasAnnotated = options.annotationDepth !== 'full_dbsnp'
      && (fs.existsSync(annotatedPath1) || fs.existsSync(annotatedPath2));

    const annotatedVCF = hasAnnotated
      ? (fs.existsSync(annotatedPath1) ? annotatedPath1 : annotatedPath2)
      : vcfPath;

    result = await analyzeVCF(annotatedVCF, {
      annotated: hasAnnotated,
      annotationDepth: options.annotationDepth ?? 'compact',
  ...
  let traitScores: Array<{ trait_id: string; score: number; confidence: number; evidenceTier?: 1 | 2 | 3 }> = [];
  ...
  try {
    ...
    if (result.clinvarAnnotations && result.clinvarAnnotations.length > 0) {
      ...
      clinvarPathogenicCount = result.clinvarAnnotations.filter((annotation: any) =>
        isStrictPathogenicClinVarSignificance(
          annotation?.clinical_significance || annotation?.clinicalSignificance || ''
        )
      ).length;
    ...
    if (result.vepAnnotations && result.vepAnnotations.size > 0) {
      ...
      for (const [key, ann] of result.vepAnnotations) {
        if (ann.gnomadAF < 0.01 && (ann.impact === 'HIGH' || ann.impact === 'MODERATE')) {
      ...
      const vepMissenseResult = enrichVEPMissenseLongevity(result.vepAnnotations, result.allGenotypes);
      if (vepMissenseResult.longevityGeneHits > 0) {
        console.log(`   VEP missense: ${vepMissenseResult.longevityGeneHits} missense variants in ${vepMissenseResult.genesFound.length} longevity genes (${vepMissenseResult.damagingCalls} damaging)`);
    ...
    curatedMarkerCount = (protocol.genomicProfile?.alerts?.length || 0) +
  ...
  try {
    ...
    if (result.allGenotypes && result.allGenotypes.size > 0) {
      try {
        const gwasResult = computeGWASHits(result.allGenotypes, getGWASRefDir());
        if (gwasResult.referencePresent) {
          ...
          console.log(`   GWAS: ${gwasResult.totalHits} associations across ${gwasResult.domains.length} domains`);
        } else {
          console.log('   GWAS: reference not found (run scripts/reference-build/build-gwas-reference.ts to build)');
        }
      } catch (gwasErr: any) {
  ...
  const output: DashboardOutput = {
    ...
    metadata: {
      ...
      matched_marker_count: protocol.source.matchedMarkerCount ?? result.variants.length,
      ...
      variant_tab_count: variantCards.genetic_conditions.length + variantCards.drug_response.length + variantCards.other_risks.length + variantCards.rare_mutations.length + variantCards.uncommon_mutations.length,
      variant_cards: variantCards,
      rsid_annotation_source: protocol.source.rsidAnnotationSource ?? result.rsidAnnotationSource,
      rsid_annotation_limitation: protocol.source.rsidAnnotationLimitation ?? result.rsidAnnotationLimitation,
      clinvar_confidence_counts: countClinVarConfidenceTiers(result.clinvarAnnotations),
  ...
  if (errors.length > 0) {
    console.log(`⚠️  Pipeline complete with ${errors.length} degraded step(s) in ${elapsed}ms`);
  ...
  if (logDir) {
    try {
      ...
      for (const s of steps) {
        if (s.status === 'error') {
          const errMsg = errors.find(e => e.step === s.step)?.error || 'unknown';
          logger.logEntry({ step: s.step, duration_ms: s.duration_ms, status: 'error', error: errMsg, timestamp: new Date().toISOString() });
        } else {
          logger.logEntry({ step: s.step, duration_ms: s.duration_ms, status: 'ok', timestamp: new Date().toISOString() });
        }
      }
      logger.close();
    } catch {
 ...
 */
export function runPipelineFromProtocol(protocol: LongevityProtocol, userId: string = 'user_001'): DashboardOutput {
  const errors: PipelineError[] = [];
  ...
  try {
    topTraits = priorities.slice(0, 5).map(p => {
      const trait = enrichedTraits.find(t => t.trait_id === p.trait_id);
      return trait
        ? { trait_id: trait.trait_id, score: trait.score, confidence: trait.confidence, mechanism: trait.mechanism || '' }
        : { trait_id: p.trait_id, score: 0, confidence: 0, mechanism: '' };
    });
  } catch (err: any) {
    errors.push({ step: 'top_traits', error: err.message });
  ...
  if (errors.length > 0) {
    console.warn(`⚠️  Pipeline completed with ${errors.length} degraded step(s):`);
    for (const e of errors) {
  ...
  for (const a of protocol.genomicProfile.alerts) if (a.gene) protocolGeneSet.add(a.gene.toUpperCase());
  for (const sp of protocol.genomicProfile.superpowers) if (sp.gene) protocolGeneSet.add(sp.gene.toUpperCase());
  let conditionCatalogMatches: CatalogMatchSummary | undefined;
  try {
  ...
  return {
    ...
    metadata: {
      ...
      cpic_actionable: 0,
      variant_tab_count: 0,
      variant_cards: { genetic_conditions: [], drug_response: [], other_risks: [], rare_mutations: [], uncommon_mutations: [] },
      prs_scores: [],
      vep_status: "skipped",
 ...
 */
export function savePipelineOutput(output: DashboardOutput, userId: string, outputDir: string): string {
...
export function printDailyActionPlanCronPrompt(outputDir: string): void {
  ...
  console.log('Ask the user: Do you want to set up a recurring cron job so your agent reviews the latest dashboard and generates a daily action plan?');
  console.log(`Dashboard context: ${dashboardPath}`);
  console.log('Only add cron after explicit opt-in, with the exact agent command, schedule, output location, and local data paths confirmed.');
}
 ...
 */
function mapCategoryGLI(categoryGli: Record<string, number>): Record<string, number> {
  ...
  for (const [key, score] of Object.entries(categoryGli)) {
    ...
    if (/cholesterol|lipid_metab|apob|pcsk9|ldlr|inflamm|cardiovascular|endothelial|blood_pressure|apoB|dna(?!_repair)/i.test(k)) {
      groups.vulnerability.push(score);
    } else if (/dna_repair|genome_stability|telomere|senescence|oncogene/i.test(k)) {
      groups.vulnerability.push(score);
    } else if (/thrombos|clotting|cancer_suscept|neurodeg|alzheimer/i.test(k)) {
      groups.vulnerability.push(score);
    } else if (/cardiomyopathy|arrhythmia|aortopathy|cardiotoxicity|long_qt/i.test(k)) {
      groups.vulnerability.push(score);
    } else if (/monogenic_diabetes|lipid_disorder|rare_functional/i.test(k)) {
      groups.vulnerability.push(score);
    } else if (/cardiomyopathy|hereditary_cancer|connective_tissue|arrhythmia/i.test(k)) {
      groups.vulnerability.push(score);
    } else if (/eye_health|kidney_health|bone_health|hearing_health|skin_health/i.test(k)) {
      groups.vulnerability.push(score);
    } else if (/neurological_health|blood_health|respiratory_health|liver_health/i.test(k)) {
      groups.vulnerability.push(score);
    }
    // Pharmacology: drug metabolism, CYP, transporters
    else if (/drug|cyp\d|pharma|metabolizer|transporter|warfarin|statin|opioid|nsaid|ppi$/i.test(k)) {
      groups.pharmacology.push(score);
    } else if (/methotre|thiopurine|fluorour|bilirubin|acetylator|clopidogrel/i.test(k)) {
      groups.pharmacology.push(score);
    } else if (/thiopurine_toxicity|fluoropyrimidine|irinotecan|malignant_hyperthermia|drug_hypersensitivity|statin_myopathy/i.test(k)) {
      groups.pharmacology.push(score);
    }
    // Hereditary: single-gene conditions
    else if (/retinitis|hepatitis|hemochr|hfe$|cystic|cftr|g6pd|factor_?[vix]|f5$|f2$|hereditary/i.test(k)) {
    ...
    else if (/methylation|caffeine|lactose|comt|bdnf|dopamine|serotonin|neurotrans|personality/i.test(k)) {
    ...
    else if (/sleep|exercise|vitamin|supplement|body_comp|caloric|fasting|muscle|aerobic|cardio_fit|vo2/i.test(k)) {
      groups.wellness.push(score);
    } else if (/protein_homeo|autophagy|mitochond|oxidative|detox|antioxid|nad|sirt|omega/i.test(k)) {
      groups.wellness.push(score);
    } else if (/glucose|insulin|baseline|wellness|immune|brain_rejuv|lipid_manage/i.test(k)) {
      groups.wellness.push(score);
    }
    ...
    // Broad health categories
      ...
      groups.wellness.push(score);
    } else if (/bone_health|eye_health|kidney_health|hearing_health|skin_health/i.test(k)) {
      groups.wellness.push(score);
  ...
  for (const [cat, scores] of Object.entries(groups)) {
    if (scores.length > 0) {
      result[cat] = Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
    } else {
...
function getCategoryMarkerCounts(): Record<string, number> {
  const counts: Record<string, number> = {
    vulnerability: 0, pharmacology: 0, hereditary: 0, traits: 0, wellness: 0, ancestry: 0,
  };
...
function generateSubitems(
  catId: string, score: number
): Array<{ name: string; status: string; status_color: StatusColor; detail: string }> {
  const rawItems = CATEGORY_SUBITEMS[catId] || [["Overall Profile", "Composite score"]];
  return rawItems.slice(0, 5).map(([name, detail], i) => {
    ...
    return { name, status: statusColorToLabel(color), status_color: color, detail: detail || "" };
  });
}
...
function buildCategories(categoryGli: Record<string, number>): Category[] {
  ...
  const descs: Record<string, string> = {
    vulnerability: "Disease-associated markers assessing polygenic risk for cardiometabolic, neurological, and inflammatory conditions.",
    pharmacology: "Pharmacogenetic markers across CYP450 family and drug transporters. These influence how your body processes medications.",
    hereditary: "Monogenic condition markers. These check for carrier status of single-gene conditions.",
    traits: "Markers related to cognition, neurotransmitter function, and behavioral tendencies.",
    wellness: "Markers covering nutrition absorption, methylation, metabolism, and inflammation.",
    ancestry: "Markers tracing your deep ancestry through Y-chromosomal and mitochondrial haplogroups.",
  };

  const defs: Array<{
    id: Category["id"]; name: string; desc: string; total: number;
    extra?: "flagged" | "interactions" | "status" | "insights" | "recommendations" | "haplogroups";
  }> = [
    { id: "vulnerability", name: "Genetic Vulnerability", total: markerCounts.vulnerability || 57, extra: "flagged",
      desc: `Analysis of ${markerCounts.vulnerability || 57} ${descs.vulnerability}` },
    { id: "pharmacology", name: "Pharmacological Compatibility", total: markerCounts.pharmacology || 44, extra: "interactions",
      desc: `${markerCounts.pharmacology || 44} ${descs.pharmacology}` },
    { id: "hereditary", name: "Hereditary Conditions", total: markerCounts.hereditary || 13, extra: "status",
      desc: `${markerCounts.hereditary || 13} ${descs.hereditary}` },
    { id: "traits", name: "Personal Traits", total: markerCounts.traits || 15, extra: "insights",
      desc: `${markerCounts.traits || 15} ${descs.traits}` },
    { id: "wellness", name: "Wellness", total: markerCounts.wellness || 61, extra: "recommendations",
      desc: `${markerCounts.wellness || 61} ${descs.wellness}` },
    { id: "ancestry", name: "Ancestry", total: markerCounts.ancestry || 14, extra: "haplogroups",
  ...
  return defs.map(def => {
    ...
    if (def.extra === "interactions") interactions = Math.max(1, Math.round(flagged / 2));
    if (def.extra === "insights") insightsCount = Math.max(1, Math.round(flagged / 3));
    if (def.extra === "recommendations") recommendations = Math.max(1, Math.round(flagged / 3));
    if (def.extra === "haplogroups") haplogroups = 2;
    if (def.extra === "flagged") statusLabel = flagged > 5 ? `${flagged} concerns` : "Low risk";
    if (def.extra === "status") statusLabel = flagged === 0 ? "All Clear" : `${flagged} flagged`;

    return {
      id: def.id, name: def.name, score, status, icon: "",
      total_markers: def.total, flagged,
      interactions,
...
export function consumerizeGeneticAction(
  ...
  hasLabContext = false,
): { title: string; why: string; steps: string[]; result: string; theme: string } {
  const raw = `${action.id ?? ''} ${action.title} ${action.description ?? ''}`;
  const medicationSafety = /drug|medication|pharmac|hla|abacavir|carbamazepine|allopurinol|contraindicat|anticonvulsant|urate-lowering|hypersensitiv/i.test(raw);
  ...
  if (medicationSafety) {
    return {
      ...
      result: 'Use your genetic result as one input to a safer medication decision',
      theme: 'medication_safety',
    };
  }

  const technicalTitle = /\b[A-Z0-9]{2,}\*?[0-9:]*\b|pathway|methylation|genomic|allele|variant|polymorphism|receptor|enzyme/i.test(action.title);
  const title = technicalTitle ? 'Put a personalized health insight into practice' : action.title;
  return {
    title,
    ...
    result: 'Learn whether this personalized step improves a measurable health signal',
    theme: `genetic_${action.id ?? title.toLowerCase().replace(/[^a-z0-9]+/g, '_')}`,
  };
}
...
function buildProtocols(pipelineProtocols: Protocol[]): RProtocol[] {
  ...
  const actual = pipelineProtocols.slice(0, 4).map((p, i) => {
    ...
    const phases: ProtocolPhase[] = (p.actions || []).slice(0, 4).map((a, j) => {
      const copy = consumerizeGeneticAction(typeof a === "string" ? { title: a } : a);
      return {
        label: ["Assessment", "Active Intervention", "Optimization", "Maintenance"][j] || `Phase ${j + 1}`,
        check: copy.title,
        done: j === 0,
        desc: copy.why,
      };
...
function getConsumerStrengthName(traitId: string): string {
  ...
  if (/lupus|hepatitis|retinitis|cancer|tumor|lymphoma|leukemia|sarcoma|melanoma|cystic|fibrosis|hemochromatosis|thalassemia|g6pd|factor_v|factor_viii|hemophilia|parkinson|alzheimer|huntington|als|muscular_dystrophy|duchenne|becker|spinocerebellar|charcot/i.test(traitId)) {
    return "";
  }
  ...
  if (traitId.includes(",") || traitId.length > 40 || /[^a-z0-9_]/.test(traitId.replace(/_/g, ""))) {
    return "";
  }
...
const PRS_CATEGORY_MAP: Record<string, { category: 'longevity' | 'wellness' | 'disease_risk' | 'metabolic' | 'inflammation' | 'cognitive'; displayName: string }> = {
  telomere_length: { category: 'longevity', displayName: 'Telomere Length' },
  epigenetic_age_grimage: { category: 'longevity', displayName: 'Epigenetic Aging (GrimAge)' },
  vo2max: { category: 'longevity', displayName: 'VO₂ Max (Cardiorespiratory Fitness)' },
  ...
  chronotype_morningness: { category: 'wellness', displayName: 'Chronotype (Morning/Evening)' },
  vitamin_d: { category: 'wellness', displayName: 'Vitamin D Levels' },
  homocysteine: { category: 'wellness', displayName: 'Homocysteine Levels' },
  alcohol_consumption: { category: 'wellness', displayName: 'Alcohol Consumption Tendency' },
  caffeine_metabolism: { category: 'wellness', displayName: 'Caffeine Metabolism' },
  reaction_time: { category: 'cognitive', displayName: 'Reaction Time' },
  cognitive_performance: { category: 'cognitive', displayName: 'Cognitive Performance' },
  ...
  crp_inflammation: { category: 'inflammation', displayName: 'C-Reactive Protein (Inflammation)' },
  il6_inflammation: { category: 'inflammation', displayName: 'IL-6 (Inflammaging)' },
  coronary_artery_disease: { category: 'disease_risk', displayName: 'Coronary Artery Disease' },
  type_2_diabetes: { category: 'disease_risk', displayName: 'Type 2 Diabetes' },
  alzheimers_disease: { category: 'disease_risk', displayName: "Alzheimer's Disease" },
...
function buildPRSExpanded(prsScores: PRSScore[]): {
  disease_risks: Array<DashboardPRSScore & { displayName: string; category: 'longevity' | 'wellness' | 'disease_risk' | 'metabolic' | 'inflammation' | 'cognitive' }>;
  longevity_traits: Array<DashboardPRSScore & { displayName: string; category: 'longevity' | 'wellness' | 'disease_risk' | 'metabolic' | 'inflammation' | 'cognitive' }>;
  wellness_traits: Array<DashboardPRSScore & { displayName: string; category: 'longevity' | 'wellness' | 'disease_risk' | 'metabolic' | 'inflammation' | 'cognitive' }>;
} {
  ...
  for (const score of prsScores) {
    const mapping = PRS_CATEGORY_MAP[score.disease] || { category: 'wellness', displayName: score.disease.replace(/_/g, ' ') };
    const entry = {
    ...
    if (mapping.category === 'disease_risk') categorized.disease_risk.push(entry);
    else if (mapping.category === 'longevity') categorized.longevity.push(entry);
    else if (mapping.category === 'inflammation') categorized.inflammation.push(entry);
    else if (mapping.category === 'metabolic') categorized.metabolic.push(entry);
    else if (mapping.category === 'cognitive') categorized.cognitive.push(entry);
  ...
  return {
    ...
    wellness_traits: [...categorized.wellness, ...categorized.metabolic, ...categorized.inflammation, ...categorized.cognitive],
  };
}
...
function buildVEPMissenseSection(output: DashboardOutput): VEPMissenseSection | undefined {
  ...
  const damagingCount = output.metadata.vep_missense_damaging
    ?? calls.filter((c: any) =>
...
function buildMissenseInterpretation(call: any): string {
  ...
  switch (call.functionalSignificance) {
    case 'damaging':
      return `Missense variant (${change}) in ${geneDesc} — predicted damaging by multiple functional predictors${call.gnomadAF > 0 ? ` (population frequency: ${(call.gnomadAF * 100).toFixed(2)}%)` : ''}. ${geneDesc} is involved in longevity and healthspan pathways.`;
    ...
    case 'benign':
      return `Common missense variant (${change}) in ${geneDesc} — predicted benign. Present in the general population at ${(call.gnomadAF * 100).toFixed(1)}% frequency.`;
    default:
...
function buildMissenseAction(call: any): string {
  switch (call.functionalSignificance) {
    case 'damaging':
      return `Consider discussing this ${call.gene} variant with a healthcare provider, especially if you have a family history of conditions related to ${call.traitId.replace(/_/g, ' ')}.`;
    case 'possibly_damaging':
      return `Monitor for relevant biomarkers related to ${call.traitId.replace(/_/g, ' ')}. This finding is not diagnostic but may inform personalized health optimization.`;
    default:
      return 'No specific action required — this appears to be a benign population variant.';
  }
}

export function transformToDashboardData(output: DashboardOutput, options: DashboardTransformOptions = {}): DashboardData {
  ...
  const wgsValidationCoverage = output.metadata.wgs_validation_coverage ?? buildWgsValidationCoverage();
  ...
  const meta: Meta = {
    ...
    ref_db: `${output.metadata.rsid_annotation_source || "GRCh37 rsID annotation"} + ClinVar + CPIC + PharmGKB`,
    ...
    analysis_quality: {
      data_handling: "Local analysis. Raw genetic data is processed on this machine unless you choose to move files elsewhere.",
      analysis_scope: "Wellness and educational report based on curated markers, ClinVar, CPIC, PRS, and aging pathway mappings. It is not a diagnosis.",
      genome_build: "GRCh37-compatible rsID annotation",
      vep_status: output.metadata.vep_status || "skipped",
      vep_note: output.metadata.vep_status === "included"
        ? `${output.metadata.vep_annotation_count || 0} variants had VEP functional annotation available. Copy number variants, large insertions/deletions, tandem repeats, and rearrangements require dedicated structural-variant analysis and are not fully interpreted here.`
      ...
      total_variants: output.metadata.variant_count || output.metadata.annotated_count || 0,
      matched_markers: output.metadata.matched_marker_count ?? output.metadata.curated_markers ?? 0,
      ...
      clinvar_not_diagnostic: "ClinVar findings are educational and require clinical confirmation before medical action.",
      clinvar_vus_note: "Variants of uncertain significance are shown as uncertain context only and are not used as medical action triggers.",
      prs_note: "Polygenic scores are directional, depend on marker coverage, and should be interpreted alongside biomarkers and clinical context.",
    },
  };

  const gliScore100 = Math.round(output.gli / 10);
  const ratingColor: Record<string, StatusColor> = {
    Excellent: "optimal", Good: "neutral", Moderate: "moderate", "Needs Work": "critical",
  };
  ...
  const whatThisMeansMessages: Record<string, string> = {
    Excellent: "Your genomic profile suggests favorable genetic variants across most longevity pathways. Your innate biology is working in your favor — focus on maintaining these advantages through evidence-backed protocols.",
    ...
    Moderate: "Your genomic profile shows several areas where targeted interventions can make a meaningful difference. The focus areas below represent your highest-impact opportunities for improving healthspan.",
    "Needs Work": "Your genomic profile has significant optimization opportunities across multiple pathways. Don't be discouraged — the specific action items below are designed to address your most impactful genetic factors systematically.",
  };
  ...
  const actionPlan: ActionItem[] = output.priorities
    ...
    .map(p => {
      ...
      const consumerAction = consumerizeGeneticAction({
        ...
        title: firstAction?.title || `Review your ${prettyDiseaseName(p.trait_id).toLowerCase()} result`,
        description: firstAction?.description,
      });
      return {
        priority: (isHigh ? "High Priority" : "Medium Priority") as ActionPriority,
  ...
  const biomarkerAnalysis = analyzeBiomarkers(options.biomarkerReadings ?? [], options.userProfile);
...
const FAVORABLE_EDGE_CATALOG: Record<string, {
  name: string; gene: string; carriedByPct: number; tier: string; benefit: string;
}> = {
  // ── Longevity / FOXO3 cluster ──────────────────────────────────────────────
  'rs2802292': { name: 'The centenarian variant', gene: 'FOXO3', carriedByPct: 11, tier: 'Established',
    benefit: 'Over-represented in people who reach their late 90s in good health. Linked to stress-resistant cells, better insulin signalling, and lower cardiovascular and cancer risk.' },
  'rs2764264': { name: 'Longevity haplotype', gene: 'FOXO3', carriedByPct: 24, tier: 'Established',
    benefit: 'Part of the FOXO3 longevity-associated haplotype. Linked to reduced insulin and IGF-1 signalling sensitivity and extended healthspan.' },
  'rs1935949': { name: 'Resilience gene', gene: 'FOXO3', carriedByPct: 28, tier: 'Established',
    benefit: 'A FOXO3 variant linked to reduced cancer mortality in long-lived individuals across multiple population studies.' },
  'rs13217795': { name: 'FOXO3 longevity allele', gene: 'FOXO3', carriedByPct: 33, tier: 'Established',
    benefit: 'Replicated across Japanese, European, and African ancestry cohorts as protective for all-cause mortality and healthy aging.' },

  // ── Cardiovascular protection ──────────────────────────────────────────────
  'rs9632884': { name: 'Cardiovascular resilience', gene: 'CDKN2B-AS1', carriedByPct: 22, tier: 'Established',
    benefit: 'A protective variant at the 9p21 locus that reduces inherited cardiovascular risk. Combined with a healthy lifestyle, a meaningful head start.' },
  'rs1333042': { name: 'Longevity pathway', gene: 'CDKN2B', carriedByPct: 19, tier: 'Emerging',
    benefit: 'Associated with extended healthspan in population cohorts. Influences cell-cycle regulation and the pace of biological aging.' },
  'rs2070744': { name: 'Endurance engine', gene: 'NOS3', carriedByPct: 16, tier: 'Established',
    benefit: 'Helps your blood vessels relax and widen, so you get a bigger blood-pressure and fitness return from aerobic exercise than most people.' },
  'rs11591147': { name: 'Natural LDL shield', gene: 'PCSK9', carriedByPct: 2, tier: 'Established',
    benefit: 'A rare loss-of-function variant that dramatically lowers LDL cholesterol from birth. Carriers have lifelong cardiovascular protection without medication.' },
  'rs28942084': { name: 'PCSK9 loss-of-function', gene: 'PCSK9', carriedByPct: 1, tier: 'Established',
    benefit: 'One of the PCSK9 inactivating variants. Reduces LDL by 15-28% naturally and is associated with a 47-88% lower risk of coronary heart disease.' },

  // ── Brain / cognitive protection ───────────────────────────────────────────
  'rs7412': { name: 'Memory shield', gene: 'APOE', carriedByPct: 14, tier: 'Established',
    benefit: 'The protective form of the Alzheimer\'s gene. Carriers have meaningfully lower lifetime risk of cognitive decline.' },
  'rs429358': { name: 'APOE2 protector', gene: 'APOE', carriedByPct: 8, tier: 'Established',
    benefit: 'The APOE e2 allele. Carriers have 40% lower risk of Alzheimer\'s disease compared to the most common e3/e3 genotype.' },

  // ── Immune / inflammatory ──────────────────────────────────────────────────
  'rs5744168': { name: 'The calm immune', gene: 'IL-10', carriedByPct: 12, tier: 'Emerging',
    benefit: 'Produces more of an anti-inflammatory signal, so your body tends to settle inflammation faster once a trigger is removed.' },
  'rs1800896': { name: 'Anti-inflammatory signal', gene: 'IL10', carriedByPct: 30, tier: 'Established',
    benefit: 'The high-expression IL-10 promoter variant. Associated with lower circulating inflammation markers and reduced autoimmune susceptibility.' },
  'rs2274567': { name: 'Complement shield', gene: 'CR1', carriedByPct: 18, tier: 'Emerging',
    benefit: 'A complement receptor variant that moderates immune activation. Linked to protection against severe infectious triggers and balanced inflammatory response.' },
  'rs3834129': { name: 'Cellular safeguard', gene: 'CASP8', carriedByPct: 14, tier: 'Emerging',
    benefit: 'A CASP8 variant associated with lower lung cancer risk. May reflect more precise regulation of programmed cell death in response to DNA damage.' },

  // ── Metabolic / insulin ────────────────────────────────────────────────────
  'rs1801282': { name: 'Insulin sensitivity edge', gene: 'PPARG', carriedByPct: 18, tier: 'Established',
    benefit: 'The Pro12Ala variant of PPARG. Carriers have improved insulin sensitivity and reduced type 2 diabetes risk, especially when physically active.' },
  'rs13266634': { name: 'Zinc transporter advantage', gene: 'SLC30A8', carriedByPct: 27, tier: 'Established',
    benefit: 'A protective variant for type 2 diabetes found in multiple large GWAS studies. The favorable allele improves pancreatic beta-cell function.' },

  // ── Physical performance ───────────────────────────────────────────────────
  'rs1815739': { name: 'Fast-twitch engine', gene: 'ACTN3', carriedByPct: 30, tier: 'Established',
    benefit: 'The intact alpha-actinin-3 form. Common in power and sprint athletes. You build fast-twitch muscle readily, which protects strength and metabolic rate with age.' },
  'rs4340': { name: 'Endurance cardiovascular type', gene: 'ACE', carriedByPct: 25, tier: 'Established',
    benefit: 'The ACE insertion allele (I/I or I/D). Associated with endurance sports performance, efficient oxygen delivery, and a favorable blood pressure response to aerobic training.' },
  'rs8192678': { name: 'Mitochondrial efficiency', gene: 'PPARGC1A', carriedByPct: 36, tier: 'Emerging',
    benefit: 'The standard PPARGC1A form that supports normal PGC-1 alpha activity. Linked to efficient mitochondrial biogenesis and training adaptability.' },
  'rs2228675': { name: 'Metabolic training adaptor', gene: 'PPARG', carriedByPct: 22, tier: 'Emerging',
    benefit: 'A PPARG variant associated with improved metabolic adaptation to endurance exercise and enhanced insulin sensitivity with training.' },

  // ── Alcohol / detox protection ─────────────────────────────────────────────
  'rs1229984': { name: 'Alcohol protection variant', gene: 'ADH1B', carriedByPct: 17, tier: 'Established',
    benefit: 'The rapid-metabolizing ADH1B allele. Associated with dramatically lower rates of alcohol dependence and reduced liver disease risk from alcohol exposure.' },

  // ── Virus resistance ───────────────────────────────────────────────────────
  'rs333': { name: 'HIV resistance variant', gene: 'CCR5', carriedByPct: 10, tier: 'Established',
    benefit: 'The CCR5-delta32 deletion. Homozygous carriers are nearly fully resistant to HIV-1 infection. Heterozygous carriers have slower disease progression if exposed.' },
};

/** Map GLI rating string → healthspan status token. */
function gliRatingToStatus(rating: string): 'optimal' | 'watch' | 'attention' | 'neutral' {
...
function riskLabelToBand(label: string): 'lower' | 'average' | 'elevated' | 'high' {
  const l = label.toLowerCase();
  if (l.includes('lower') || l.includes('reduced')) return 'lower';
  if (l.includes('significantly elevated') || l.includes('high')) return 'high';
  if (l.includes('elevated') || l.includes('slightly elevated')) return 'elevated';
  return 'average';
}
...
function normalizeDashboardBiomarkerReadings(readings: DashboardBiomarkerInput[] = []): BiomarkerReading[] {
...
function translateCanonicalPlanForTemplate(plan: PersonalizedActionPlan): TemplatePlanTranslation {
  const priorities: TemplatePlanPriority[] = plan.priorities.map((action, index) => ({
    ...
    evidence: action.evidence_chips.map(chip => ({
      k: chip.source_label ? `${chip.label} · ${chip.source_label}` : chip.label,
      v: chip.target ? `${chip.value} · target ${chip.target}` : chip.value,
      sev: chip.severity === 'bad' ? 'bad' : chip.severity === 'warn' ? 'warn' : 'good',
    })),
  ...
  const reviewItems: TemplatePlanReviewItem[] = plan.review_items.map((item: PlanReviewItem) => ({
    id: item.id,
    reason: item.reason,
    title: item.title,
...
export function buildDashboardJSON(
  output: {
    ...
    category_gli: Record<string, number>;
    traits: Array<{
      trait_id: string; score: number; confidence: number;
      mechanism?: string; actions?: Array<{ title: string; description?: string }>;
    }>;
    priorities: Array<{ trait_id: string; priority: number; reasoning: string }>;
    protocols: Array<{ title: string; description?: string; actions?: Array<{ id?: string; title: string; impact: number; difficulty: string; description?: string }> }>;
    insights: Array<{ title: string; summary: string; actions?: Array<{ title: string } | string> }>;
    hallmark?: {
      hallmarks: Array<{
        hallmark_id: string; name: string; color: string;
        gene_count: number; genes: string[]; burden: number;
        actionability: string; actions: string[];
      }>;
      total_genes_hit: number; hallmarks_affected: number; summary: string;
    };
    metadata: {
      prs_scores?: DashboardPRSScore[];
      variant_cards?: {
        genetic_conditions?: Array<{ gene: string; rsid: string; disease: string; clinicalSignificance: string; annotation: string; zygosity?: string; confidenceTier?: ClinVarConfidenceTier; confidenceLabel?: string }>;
        drug_response?: Array<{ gene: string; rsid: string; disease: string; clinicalSignificance: string; annotation: string; zygosity?: string; confidenceTier?: ClinVarConfidenceTier; confidenceLabel?: string }>;
        other_risks?: Array<{ gene: string; rsid: string; disease: string; clinicalSignificance: string; annotation: string; zygosity?: string; confidenceTier?: ClinVarConfidenceTier; confidenceLabel?: string }>;
      };
      variant_count?: number; annotated_count?: number; matched_marker_count?: number; curated_markers?: number; trait_count?: number;
      rsid_annotation_source?: string;
      rsid_annotation_limitation?: string;
      clinvar_confidence_counts?: Partial<Record<ClinVarConfidenceTier, number>>;
      prs_count?: number;
  ...
  options: {
    ...
    wearableReadings?: Array<{ metric: string; value: number; unit?: string; date?: string; window_days?: number }>;
    userProfile?: UserProfile;
    /**
  ...
  const initials = userId.slice(0, 2).toUpperCase().replace(/[^A-Z]/g, '').padEnd(2, 'X').slice(0, 2);

  const gliScore100 = Math.min(100, Math.max(0, Math.round(output.gli / 10)));
  const gliStatus = gliRatingToStatus(output.gli_rating);

  // ── Signature edges ──────────────────────────────────────────────────────
  const protectiveTraits = output.traits.filter(t => t.trait_id.startsWith('protective_') && t.score >= 70);
  ...
  if (edges.length === 0) {
    edges.push({
      name: 'Genomic resilience', gene: 'Multiple', rsid: 'n/a',
      carriedByPct: 100, tier: 'Established',
      benefit: 'Your genome was analyzed across hundreds of longevity-relevant pathways. Your full genetic edge profile requires WGS with matched ClinVar protective variant coverage.',
    });
  }

  // ── Pillars (strength across modalities) ─────────────────────────────────
  const currentBiomarkerReadings = normalizeDashboardBiomarkerReadings(options.biomarkerReadings);
  const previousBiomarkerReadings = normalizeDashboardBiomarkerReadings(options.previousBiomarkerReadings);
  const hasBiomarkers = currentBiomarkerReadings.length > 0;
  const hasWearables = (options.wearableReadings?.length ?? 0) > 0;
  const wgsValidationCoverage = output.metadata.wgs_validation_coverage ?? buildWgsValidationCoverage();
  const biomarkerAnalysis = analyzeBiomarkers(currentBiomarkerReadings, options.userProfile);
  const previousBiomarkerAnalysis = previousBiomarkerReadings.length > 0
    ? analyzeBiomarkers(previousBiomarkerReadings, options.userProfile)
    : undefined;
  const biomarkerTrends = buildBiomarkerTrendMap(biomarkerAnalysis, previousBiomarkerAnalysis);
  const wearableAnalysis = analyzeWearables((options.wearableReadings ?? []).map(reading => ({
    id: reading.metric,
    value: reading.value,
    unit: reading.unit,
  })));
  ...
  const pillars: Array<{
    modality: string; secondary: boolean;
    score: number; scoreLabel: string; scoreStatus: string;
    primary: string;
  ...
  }> = [
    {
      ...
      primary: edges.length > 1
        ? `You carry ${edges.length} standout variants, including ${edges[0]!.name.toLowerCase()}.`
        : 'Your genomic profile has been analyzed across key longevity pathways.',
      supports: genesSupports.length > 0 ? genesSupports : [{ lbl: 'Genomic score', val: `${gliScore100}/100` }],
    },
  ];
  ...
  if (hasBiomarkers) {
    pillars.push({
      ...
      scoreLabel: 'BIO',
      scoreStatus: biomarkerAnalysis.score >= 80 ? 'optimal' : biomarkerAnalysis.score >= 55 ? 'watch' : 'attention',
      primary: `Blood panel has been imported and analyzed across ${biomarkerAnalysis.domains.filter(d => d.measured > 0).length} body systems.`,
      supports: biomarkerAnalysis.findings.slice(0, 4).map(b => ({
        lbl: b.name,
        val: `${b.display_value ?? `${b.value} ${b.unit}`} · ${b.status.replace(/_/g, ' ')}`,
      })),
    });
  } else {
    pillars.push({
      ...
      scoreLabel: 'BIO',
      scoreStatus: 'neutral',
      primary: 'Add a blood panel to see how your current biomarkers compare to your genetic baseline.',
  ...
  if (hasBiomarkers) subscores.push({ id: 'biomarker', value: biomarkerAnalysis.score, label: 'Blood work', status: biomarkerAnalysis.status === 'needs_attention' ? 'attention' as any : biomarkerAnalysis.status as any });
  if (hasWearables)  subscores.push({ id: 'wearable', value: wearableAnalysis.score ?? 60, label: 'Behavioral', status: (wearableAnalysis.status ?? 'watch') as any });

  const connected = [
    { name: 'Whole-genome sequence', connected: true, count: `${output.metadata.variant_count || 0} variants` },
    { name: 'Blood panel', connected: hasBiomarkers, count: hasBiomarkers ? `${biomarkerAnalysis.lab_data?.length ?? options.biomarkerReadings!.length} lab values` : 'Not connected' },
    { name: 'Behavioral', connected: hasWearables, count: hasWearables ? `${options.wearableReadings!.length} signals` : 'Not connected' },
  ];

  // ── Tracking (Overview marker groups) ────────────────────────────────────
  const metabolicCategories = ['lipid', 'glucose', 'cardiovascular', 'blood', 'body'];
  const inflammationCategories = ['inflammation', 'immune', 'oxidative', 'methylation'];
  const buildTrackingFromCategories = (cats: string[], title: string, id: string) => {
    ...
    return {
      ...
      summary: traits.length > 0
        ? (output.insights.find(i => cats.some(c => i.title.toLowerCase().includes(c)))?.summary || 'Based on your integrated wellness analysis.')
        : 'No specific variants found in this category.',
      markers: traits.map(t => ({
        name: prettyDiseaseName(t.trait_id),
        tech: '',
        value: `score ${t.score}/100`,
        status: t.score >= 70 ? 'optimal' : t.score >= 45 ? 'watch' : 'attention',
      })),
    };
  };

  const tracking = [
    buildTrackingFromCategories(metabolicCategories, 'Heart & metabolism', 'metabolic'),
    buildTrackingFromCategories(inflammationCategories, 'Inflammation', 'inflammation'),
    {
      id: 'recovery',
      title: 'Day-to-day behavioral',
      score: hasWearables ? (wearableAnalysis.score ?? 0) : 0,
      scoreStatus: (hasWearables ? (wearableAnalysis.status ?? 'watch') : 'neutral') as 'optimal' | 'watch' | 'attention' | 'neutral' | 'missing',
      isSecondary: true,
      summary: hasWearables
        ? (wearableAnalysis.findings.find(f => f.status !== 'optimal')?.interpretation ?? 'Behavioral data is connected and within healthy ranges.')
      ...
      markers: hasWearables
        ? wearableAnalysis.findings.slice(0, 5).map(f => ({
            ...
            status: f.status as 'optimal' | 'watch' | 'attention' | 'needs_attention',
          }))
        : [],
    },
  ];
  ...
  const canonicalPlan = options.personalizedActionPlan
  ...
  ].filter(v => !v.clinicalSignificance.toLowerCase().includes('benign')).length;
  ...
  const annotatedRsidCount = localCoverage?.unique_rsids ?? output.metadata.annotated_count ?? 0;
  const matchedMarkerCount = output.metadata.matched_marker_count ?? curatedMarkers;
  const geneticStats = [
    { value: formatVariantCount(wgsRecordCount), label: 'DNA positions read' },
    { value: formatVariantCount(annotatedRsidCount), label: 'Annotated rsIDs available' },
    { value: formatVariantCount(matchedMarkerCount), label: 'Curated marker rules applied' },
    { value: String(clinvarActionable), label: 'Clinician-review findings surfaced' },
  ];
  const genomicCoverage = {
    headline: localCoverage
      ? `${formatVariantCount(localCoverage.total_records)} DNA positions were read across ${localCoverage.classes_present} variant classes.`
    ...
    interpreted: [
      `${formatVariantCount(matchedMarkerCount)} curated marker rules used for wellness and trait interpretation`,
      `${formatVariantCount(localCoverage?.unique_rsids ?? output.metadata.annotated_count)} annotated rsIDs available for ClinVar, CPIC, PRS, and pathway matching`,
      `${formatVariantCount(clinvarActionable)} non-benign hereditary or medication findings promoted to consumer cards`,
    ],
    classes: [
      {
        ...
        meaning: 'Used for nutrition, longevity traits, medication response, inherited-risk flags, and polygenic scores.',
      },
      {
        label: 'Small insertions and deletions',
        count: recordsByClass.indel ?? 0,
        status: 'Counted and selectively interpreted',
        meaning: 'Can affect protein sequence or gene regulation; consumer findings are shown only when annotations are strong enough.',
      },
      {
        ...
        meaning: 'Can change gene dosage. These need dedicated CNV validation before they become personal health recommendations.',
      },
      {
        label: 'Large indels and rearrangements',
        count: (recordsByClass.large_indels ?? 0) + (recordsByClass.rearrangements ?? 0),
        status: 'Coverage visible, specialist review needed',
        meaning: 'Can alter larger genome regions. The dashboard reports readiness until local SV calling and external validation are complete.',
      },
      {
        label: 'Tandem repeats',
        count: recordsByClass.tandem_repeats ?? 0,
        status: 'Detected as a class',
        meaning: 'Repeat-length findings need purpose-built callers before they should be translated into consumer health guidance.',
      },
  ...
  const polygenic = (output.metadata.prs_scores || [])
    ...
    .map(p => ({
      ...
      desc: [
        p.description || '',
        p.sourceName ? `Source: ${p.sourceName}${p.sourceId ? ` (${p.sourceId})` : ''}.` : '',
        p.coveragePct != null ? `Coverage: ${p.variantsScored ?? 0}/${p.totalWeightedVariants ?? 0} variants (${p.coveragePct}%).` : '',
      ].filter(Boolean).join(' '),
    }));
  ...
  const strengths = strengthTraits.map(t => {
    const relatedEdge = Object.entries(FAVORABLE_EDGE_CATALOG).find(([, e]) =>
      e.gene && t.mechanism?.includes(e.gene)
    );
    return {
      ...
      score: t.score,
      body: t.mechanism || `Score: ${t.score}/100 — this genetic pathway is working in your favor.`,
      tags: [t.score >= 85 ? 'High impact' : 'Moderate impact', t.confidence >= 0.85 ? 'Strong evidence' : 'Moderate evidence'],
    };
  });
  ...
  const hereditaryVariants = [
    ...
    .filter(v => {
      ...
      return sig.includes('pathogenic') || sig.includes('risk factor') || sig.includes('carrier');
    })
    .slice(0, 5)
    .map(v => ({
      gene: v.gene || 'Unknown',
      ...
      lead: v.zygosity ? `${v.zygosity.replace('Homozygous', 'Two copies').replace('Heterozygous', 'One copy')}.` : 'Variant detected.',
      desc: v.annotation?.split('—')[1]?.trim() || 'Discuss with a clinician.',
      tag: v.clinicalSignificance.includes('Pathogenic') ? 'Pathogenic' : 'Risk factor',
  ...
  const drugGene = (output.metadata.variant_cards?.drug_response || [])
    .filter(v => {
      ...
      return !gene.includes('not_provided') && gene.length > 2 && v.disease && v.disease !== 'Not specified';
    })
    .slice(0, 4)
    .map(v => ({
      gene: v.disease || 'Drug response',
      rsid: `${v.gene} · ${v.rsid}`,
      name: `How your body processes ${v.disease?.replace(' response', '') || 'this drug'}`,
      lead: 'Variant detected.',
      desc: v.annotation?.split('—')[1]?.trim() || 'Discuss with prescribing physician before starting new medications.',
      tag: 'Heads-up',
      confidence: v.confidenceLabel || confidenceTierLabel(v.confidenceTier),
    }));
  ...
  const biomarkersInTarget = biomarkerAnalysis.findings.filter(b => b.status === 'optimal').length;
  const actionableBiomarkers = biomarkerAnalysis.findings.filter(b => b.status !== 'optimal');
  const biomarkerStats = [
    { value: hasBiomarkers ? `${biomarkersInTarget}/${biomarkerFindingCount}` : '--', label: 'Markers in target' },
    { value: String(biomarkerAnalysis.lab_data?.length ?? currentBiomarkerReadings.length), label: 'Direct lab values analyzed' },
    { value: hasBiomarkers ? String(biomarkerAnalysis.derived_biomarkers?.length ?? 0) : '--', label: 'Derived biomarkers calculated' },
    { value: hasBiomarkers ? String(biomarkerAnalysis.score) : '--', label: 'Bloodwork score out of 100' },
    { value: hasBiomarkers ? String(actionableBiomarkers.length) : '--', label: 'Priority markers to review' },
    { value: hasBiomarkers ? dateStr : 'Not connected', label: hasBiomarkers ? 'Date of data import' : 'Connect blood panel' },
  ];

  const priorityFindings = actionableBiomarkers.slice(0, 3);
  const biomarkerPriority = hasBiomarkers && priorityFindings.length > 0
    ? {
  ...
  const wearableFindingCount = hasWearables ? wearableAnalysis.findings.length : 0;
  ...
  const actionableWearables = hasWearables
    ? wearableAnalysis.findings.filter(f => f.status !== 'optimal').length
    : 0;
  const wearableWindows = Array.from(new Set(
  ...
  const wearableWindowLabel = hasWearables
    ...
    : '--';
  const wearableStats = [
    { value: hasWearables ? `${wearablesInTarget}/${wearableFindingCount}` : '--', label: 'Signals in target' },
    { value: String(wearableAnalysis.measured_count ?? 0), label: 'Signals tracked' },
    { value: hasWearables ? String(wearableAnalysis.score ?? 0) : '--', label: 'Behavioral score out of 100' },
    { value: hasWearables ? String(actionableWearables) : '--', label: 'Priority signals to review' },
  ...
  const wearableDomains = hasWearables
    ? wearableAnalysis.domains
        ...
        .sort((a, b) => a.score - b.score)
        .map(d => {
          const domainFindings = wearableAnalysis.findings.filter(f => f.domain === d.id);
          ...
          return {
            ...
            summary: topFinding
              ...
              : `${d.measured} signal${d.measured !== 1 ? 's' : ''} measured — all within healthy ranges.`,
  ...
  const dashboardData = {
    ...
    quality: {
      ...
      rsid_annotation_source: output.metadata.rsid_annotation_source,
      rsid_annotation_note: output.metadata.rsid_annotation_limitation,
      clinvar_confidence_counts: output.metadata.clinvar_confidence_counts ?? {},
      clinvar_not_diagnostic: "ClinVar findings are educational and require clinical confirmation before medical action.",
      clinvar_vus_note: "Variants of uncertain significance are shown as uncertain context only and are not used as medical action triggers.",
    },
    ...
    identity: { pillars },
    healthspan: { gli: gliScore100, gliStatus, percentile: gliScore100, focusAreas, subscores, connected },
    tracking,
    plan: {
...
function buildBiomarkerTrendMap(
  ...
  const trends = new Map<string, { trend_delta: number; trend_label: string }>();
...
function buildBiomarkerCategoriesFromAnalysis(
  ...
  title: string; score: number; scoreStatus: string; summary: string;
  markers: Array<{
    name: string; tech: string; value: string; status: string; statusLabel: string;
    targetLabel: string; priorityRank?: number; trendLabel?: string; trendDelta?: number; info: string;
  }>;
}> {
  const statusFor = (status: string): 'optimal' | 'watch' | 'attention' | 'neutral' => {
  ...
  const valuesById = new Map([
    ...(analysis.lab_data ?? []).map(value => [value.id, value.display_value] as const),
    ...(analysis.derived_biomarkers ?? []).map(value => [value.id, value.display_value] as const),
  ]);
  const findingById = new Map(analysis.findings.map(finding => [finding.id, finding]));
  const rowsFor = (ids: string[]) => ids
    .map(id => findingById.get(id))
    .filter((finding): finding is BiomarkerFinding => Boolean(finding))
    .map(finding => ({
      name: finding.name,
      tech: finding.source_type === 'derived' ? 'Calculated' : 'Lab result',
      value: valuesById.get(finding.id) ?? finding.display_value ?? `${finding.value} ${finding.unit}`,
      ...
      info: finding.source_type === 'derived'
  ...
  const domainGroups = analysis.domains
    ...
    .map(domain => {
      const ids = analysis.findings
        .filter(finding => finding.domain === domain.id && finding.source_type !== 'derived')
        .map(finding => finding.id);
      return {
        title: domain.name,
        score: domain.score,
        scoreStatus: statusFor(domain.status),
        summary: `${rowsFor(ids).length} marker${rowsFor(ids).length === 1 ? '' : 's'} analyzed in this body system, ordered by distance from target.`,
        markers: rowsFor(ids),
      };
    })
  ...
  return [
    ...
    derivedOnly.length > 0 ? {
      ...
      score: derivedOnly.length
        ? Math.round(derivedOnly.reduce((sum, item) => sum + (item.status === 'optimal' ? 90 : item.status === 'watch' ? 60 : 35), 0) / derivedOnly.length)
        : 0,
      scoreStatus: derivedOnly.some(item => item.status === 'attention') ? 'attention' : derivedOnly.some(item => item.status === 'watch') ? 'watch' : 'optimal',
      summary: `${derivedOnly.length} calculated value${derivedOnly.length === 1 ? '' : 's'} derived from the direct labs.`,
      markers: derivedOnly,
    } : null,
...
function buildBiomarkerCategoriesFromReadings(
  readings: Array<{ name: string; value: number; unit: string; referenceMin?: number; referenceMax?: number; flag?: string }>
): Array<{
  title: string; score: number; scoreStatus: string; summary: string;
  markers: Array<{ name: string; tech: string; value: string; status: string; info: string }>;
}> {
  const categories: Array<{ title: string; keywords: string[] }> = [
    { title: 'Heart & cholesterol',     keywords: ['cholesterol', 'ldl', 'hdl', 'triglyceride', 'apob', 'lpa', 'homocysteine', 'lipid'] },
    { title: 'Blood sugar & insulin',   keywords: ['glucose', 'hba1c', 'insulin', 'homa', 'a1c'] },
    { title: 'Inflammation & immune',   keywords: ['crp', 'ferritin', 'esr', 'wbc', 'albumin', 'inflammation', 'immune'] },
    { title: 'Vitamins & nutrients',    keywords: ['vitamin', 'omega', 'folate', 'b12', 'magnesium', 'zinc', 'selenium'] },
    { title: 'Liver',                   keywords: ['alt', 'ast', 'ggt', 'alp', 'bilirubin', 'liver'] },
    { title: 'Kidney & electrolytes',   keywords: ['creatinine', 'egfr', 'bun', 'sodium', 'potassium', 'kidney'] },
    { title: 'Thyroid & hormones',      keywords: ['tsh', 'thyroid', 'testosterone', 'cortisol', 'estrogen', 'hormone'] },
    { title: 'Blood count',             keywords: ['hemoglobin', 'hematocrit', 'rbc', 'platelet', 'neutrophil', 'lymphocyte', 'cbc'] },
  ];

  const statusFromFlag = (flag?: string): 'optimal' | 'watch' | 'attention' | 'neutral' => {
  ...
  return categories.map(cat => {
    ...
    const scores = matched.map(r => statusFromFlag(r.flag) === 'optimal' ? 80 : statusFromFlag(r.flag) === 'watch' ? 55 : 30);
    const avgScore = Math.round(scores.reduce((s, v) => s + v, 0) / scores.length);
    const scoreStatus = avgScore >= 70 ? 'optimal' : avgScore >= 50 ? 'watch' : 'attention';

    return {
      title: cat.title,
      score: avgScore,
      scoreStatus,
      summary: `${matched.length} marker${matched.length !== 1 ? 's' : ''} in this category.`,
      markers: matched.map(r => ({
        name: r.name,
        tech: '',
        value: `${r.value} ${r.unit}`,
        status: statusFromFlag(r.flag),
        info: r.flag && r.flag !== 'N' && r.flag !== 'normal'
          ? `This marker is outside its reference range (${r.referenceMin ?? '?'} - ${r.referenceMax ?? '?'} ${r.unit}). Discuss with your clinician.`
...
function buildWearableDomainsFromReadings(
  ...
  const domains: Array<{ title: string; keywords: string[] }> = [
    { title: 'Cardiovascular recovery', keywords: ['hrv', 'rhr', 'heart', 'spo2', 'oxygen', 'recovery'] },
    { title: 'Sleep & rest',            keywords: ['sleep', 'deep', 'rem', 'awake', 'bedtime'] },
    { title: 'Movement & training',     keywords: ['steps', 'active', 'calories', 'workout', 'zone', 'training'] },
    { title: 'Daily rhythm',            keywords: ['strain', 'stress', 'readiness', 'variability'] },
  ];
  ...
  return domains.map(d => {
    ...
    return {
      ...
      score: 60,
      scoreStatus: 'watch',
      summary: `${matched.length} signal${matched.length !== 1 ? 's' : ''} connected in this domain.`,
      markers: matched.map(r => ({
        name: r.metric,
        tech: '',
...
(async function main() {
  const { runHealthAnalysis, saveHealthAnalysisOutput, summarizeRunForConsole } = await import('./health_analysis.js');
  const { runInputDoctor, renderDoctorReport } = await import('./input_doctor.js');

  const HELP_TEXT = `
🧬 foreverbetter Health Pipeline

Usage:
  npm run pipeline -- --genetics=path [--biomarkers=path] [--biomarkers-previous=path] [--wearables=path] [--user=id] [--out=dir]
...
Supported flags:
  ...
  --design=<id|path>            Dashboard layout: dossier (the full all-modality
                                Healthspan dossier), ring-data, performance,
  ...
  --annotation-depth=<mode>     compact (default) or full_dbsnp
  --dbsnp-path=<path>           Full dbSNP GRCh37 VCF.GZ path for full_dbsnp
...
`;
  ...
  if (positional.length > 0) {
    ...
    console.error('');
    console.error(`   You supplied: ${positional.map(p => `"${p}"`).join(' ')}`);
    console.error('');
    console.error('   Use named flags instead:');
    console.error('     npm run pipeline -- --genetics=path --biomarkers=path --wearables=path --user=id --out=dir');
    console.error('');
    console.error('   Examples:');
    console.error('     npm run pipeline -- --genetics=/path/to/file.vcf.gz --user=user_001 --out=./output');
    console.error('     npm run pipeline -- --biomarkers=examples/sample-biomarkers.csv --wearables=examples/sample-whoop-api.json');
    console.error('');
    console.error('   See `npm run pipeline -- --help` for the full flag list.');
    process.exit(2);
  }
  ...
  const annotationDepth = flag('annotation-depth') as 'compact' | 'full_dbsnp' | undefined;
  const dbsnpPath = flag('dbsnp-path');
  if (annotationDepth && annotationDepth !== 'compact' && annotationDepth !== 'full_dbsnp') {
    console.error('❌ --annotation-depth must be compact or full_dbsnp.');
    process.exit(2);
  }
  if (annotationDepth === 'full_dbsnp' && !dbsnpPath) {
    console.error('❌ --dbsnp-path is required with --annotation-depth=full_dbsnp.');
    process.exit(2);
  }

  // Reject unknown flags so a typo never silently runs the wrong thing.
  const KNOWN_FLAGS = new Set(['--genetics', '--biomarkers', '--biomarkers-previous', '--wearables', '--user', '--out', '--design', '--annotation-depth', '--dbsnp-path', '--doctor', '--help', '-h']);
  for (const arg of args) {
    ...
    console.error('   Run `npm run pipeline -- --help` for the supported flag list.');
    process.exit(2);
  }
  ...
  if (doctorMode) {
    const report = runInputDoctor({ geneticsPath, biomarkersPath, previousBiomarkersPath, wearablesPath });
  ...
  if (!geneticsPath && !biomarkersPath && !wearablesPath) {
    ...
    console.error('   Supply at least one of --genetics, --biomarkers, --wearables.');
  ...
  const preflight = runInputDoctor({ geneticsPath, biomarkersPath, previousBiomarkersPath, wearablesPath });
  ...
  if (previousBiomarkersPath) supplied.push(`biomarkers-previous=${previousBiomarkersPath}`);
  ...
  try {
    ...
    if (result.genomic_output) {
      ...
      const template = injectTheme(fs.readFileSync(templatePath, 'utf-8'), process.argv.find(a => a.startsWith('--design='))?.split('=')[1]);
      const biomarkerReadings = biomarkersPath ? parseBiomarkerFile(biomarkersPath) : [];
      const previousBiomarkerReadings = previousBiomarkersPath ? parseBiomarkerFile(previousBiomarkersPath) : [];
      const wearableReadings = wearablesPath ? parseWearableFile(wearablesPath) : [];
      ...
      const dashboardData = transformToDashboardData(output, { biomarkerReadings, previousBiomarkerReadings, wearableReadings });
      // Attach the canonical plan so M4 / downstream consumers can render it directly.
      (dashboardData as { personalized_action_plan?: typeof result.plan }).personalized_action_plan = result.plan;
      ...
      const styledDesign = process.argv.find(a => a.startsWith('--design='))?.split('=')[1];
      // The `dossier` design IS the full deep "Healthspan dossier" dashboard, so
      ...
      const styled = styledDesign === 'dossier' ? html : renderFullDashboard(JSON.parse(dashboardJson), styledDesign);
      ...
      console.log(`📄 Dashboard saved: ${htmlPath} (design: ${styledDesign ?? 'clinical-modern'}; deep genomic view: deep-dive.html)`);
      ...
      console.log('ℹ️  Genetics not supplied — rendering modality-optional dashboard.');
      ...
      const dashboardData = transformToDashboardData(stub, { biomarkerReadings, previousBiomarkerReadings, wearableReadings });
      (dashboardData as { personalized_action_plan?: typeof result.plan }).personalized_action_plan = result.plan;
      const html = renderDashboard(template, dashboardData, dashboardJson);
      // Primary dashboard = the chosen distinct design layout. The deep genomic
      // template is preserved as deep-dive.html.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
    } else if (msg.includes('ENOSPC') || msg.includes('disk') || msg.includes('space')) {
      console.error(`
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
