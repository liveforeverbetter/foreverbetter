#!/usr/bin/env node
/**
 * Vet every design end to end: render all six from the same data and confirm
 * each produces a STRUCTURALLY unique dashboard (different hero component,
 * sections, and voice), not just a recolor.
 *
 *   npm run design:vet                 # writes output/designs/<id>.html for all six
...
function toDashboardData(): DashboardData {
  if (jsonArg) {
    ...
    const cards = (raw.cards ?? raw.dashboard_spec?.cards ?? []).map((c: any) => ({
      title: c.title, score: c.score, status: c.status, summary: c.summary, action: c.action, category: c.category,
    }));
    return { score: raw.healthspan?.gli ?? raw.score, summary: raw.summary, cards, priorities: raw.plan?.priorities, coverage: raw.coverage };
  }
  return {
    score: 74,
    ...
    coverage: [
      ...
      { modality: 'genetics', label: 'Genetics', signal_count: 0, status: 'not_provided' },
    ],
    cards: [
      { title: 'ApoB', score: 55, status: 'needs_attention', summary: 'Above the optimal range for cardiovascular risk.', action: 'Review ApoB-lowering options with a clinician.' },
      { title: 'HbA1c', score: 68, status: 'watch', summary: 'Trending toward the upper range.', action: 'Tighten the metabolic routine and retest in 90 days.' },
      ...
      { title: 'Vitamin D', score: 48, status: 'low', summary: 'Below the target range.', action: 'Supplement and recheck.' },
      { title: 'Triglycerides', score: 60, status: 'watch', summary: 'Borderline.' },
      { title: 'Activity', score: 65, status: 'watch', summary: 'Below your movement floor.', action: 'Add a daily step target.' },
    ],
    priorities: [
...
for (const id of DESIGN_IDS) {
  ...
  const bleed = DESIGN_IDS.filter(other => other !== id && SIGNATURE[id].test(rendered[other]));
  const uniqueLen = new Set(Object.values(rendered).map(h => h.length)).size === DESIGN_IDS.length;
  const status = sigOk && bleed.length === 0 ? 'OK ' : 'FAIL';
  if (status === 'FAIL') ok = false;
  console.log(`  [${status}] ${id.padEnd(16)} signature=${sigOk ? 'present' : 'MISSING'}  bleed=${bleed.length ? bleed.join(',') : 'none'}  bytes=${html.length}`);
}
console.log(`\nAll six lengths distinct: ${new Set(Object.values(rendered).map(h => h.length)).size === DESIGN_IDS.length}`);
console.log(`Written to ${path.relative(process.cwd(), outDir)}/`);
if (!ok) { console.error('\nVET FAILED: a design is missing its signature or bleeds into another.'); process.exit(1); }
console.log('\nVET PASSED: all six designs are structurally unique.');
