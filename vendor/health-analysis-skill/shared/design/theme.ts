...
export interface DesignTokens {
  ...
  typography: { font_display: string; font_body: string; google_fonts?: string[] };
...
export function listBundledSystems(): Array<{ id: string; name: string; scheme: string; best_for?: string[]; layout?: unknown; modalities?: string[]; metric_ids?: string[] }> {
  return bundled().map(s => ({
    ...
    modalities: Array.from(new Set(((s as any).modality_sections ?? []).map((section: any) => section.modality))),
...
export function resolveTheme(idOrTokens?: string | DesignTokens): DesignTokens {
  ...
  return systems.find(s => s.id === idOrTokens) ?? systems.find(s => s.id === DEFAULT_DESIGN_ID)!;
}
...
export function dashboardTemplateOverride(theme: DesignTokens): { css: string; fontsHref: string | null } {
  ...
  const lines = Object.entries(map).filter(([, v]) => v).map(([k, v]) => `  --${k}: ${v};`);
  const css = `:root {\n  color-scheme: ${theme.scheme};\n${lines.join('\n')}\n}`;
  const fonts = t.google_fonts ?? [];
  const fontsHref = fonts.length ? `https://fonts.googleapis.com/css2?${fonts.map(f => `family=${f}`).join('&')}&display=swap` : null;
...
export function injectTheme(templateHtml: string, design?: string): string {
  ...
  const inject = `${fontsHref ? `<link rel="stylesheet" href="${fontsHref}">\n` : ''}<style id="theme-override" data-design="${theme.id}">\n${css}\n</style>\n</head>`;
  return templateHtml.includes('</head>') ? templateHtml.replace('</head>', inject) : `${templateHtml}\n<style id="theme-override">\n${css}\n</style>`;
}

// Emit a `:root { ... }` CSS block plus an optional Google Fonts <link>.
export function themeCss(theme: DesignTokens): { rootCss: string; fontsHref: string | null; scheme: string } {
  const c = theme.colors;
  const flat = (v: string | string[]) => (Array.isArray(v) ? v.join(', ') : v);
  const vars: string[] = [];
  for (const [k, v] of Object.entries(c)) vars.push(`  --${k.replace(/_/g, '-')}: ${flat(v)};`);
  vars.push(`  --font-display: ${theme.typography.font_display};`);
  vars.push(`  --font-body: ${theme.typography.font_body};`);
  for (const [k, v] of Object.entries(theme.radii)) vars.push(`  --radius-${k}: ${v};`);
