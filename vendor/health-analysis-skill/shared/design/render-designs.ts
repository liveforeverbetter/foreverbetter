...
export interface DashboardData {
  ...
  summary?: string;
  coverage?: Array<{ modality: string; label: string; signal_count: number; status: string }>;
  cards: Array<{ title: string; score?: number; status?: string; summary?: string; action?: string; category?: string }>;
...
const STATUS_VAR: Record<string, string> = {
  optimal: 'positive', good: 'positive', ok: 'positive', in_range: 'positive',
  watch: 'warning', upcoming: 'warning', borderline: 'warning',
  needs_attention: 'negative', high: 'negative', low: 'negative', out_of_range: 'negative',
  missing: 'text-muted', not_connected: 'text-muted', not_provided: 'text-muted',
};
const statusColor = (s?: string) => `var(--${STATUS_VAR[String(s ?? '').toLowerCase()] ?? 'primary'})`;
const clampPct = (n?: number) => Math.max(3, Math.min(100, Number(n ?? 0)));
const num = (n?: number) => (n == null ? '--' : String(Math.round(n)));

interface DesignVoice { scoreWord: string; heroKicker: string; planTitle: string; tone: 'editorial' | 'coach' | 'clinical' | 'data' | 'system' | 'calm'; }
const VOICE: Record<string, DesignVoice> = {
  'ring-data': { scoreWord: 'Readiness', heroKicker: 'Today', planTitle: 'Gentle next steps', tone: 'editorial' },
  'performance': { scoreWord: 'Recovery', heroKicker: "TODAY'S READINESS", planTitle: 'FOCUS', tone: 'coach' },
  'clinical-modern': { scoreWord: 'Healthspan', heroKicker: 'Panel summary', planTitle: 'Clinical priorities', tone: 'clinical' },
  'metabolic': { scoreWord: 'Metabolic', heroKicker: 'Your signal', planTitle: 'Protocol', tone: 'data' },
  'system-cards': { scoreWord: 'Overall', heroKicker: 'Summary', planTitle: 'Suggestions', tone: 'system' },
  'serene': { scoreWord: 'Balance', heroKicker: 'Where you are', planTitle: 'One thing at a time', tone: 'calm' },
};
...
function coverageStrip(d: DashboardData, cls = 'cov'): string {
  const parts = (d.coverage ?? []).filter(c => c.status === 'connected' && c.signal_count > 0)
    .map(c => `<span>${esc(c.signal_count)} ${esc(String(c.label).toLowerCase())}</span>`);
  return parts.length ? `<div class="${cls}">${parts.join('<i>·</i>')}</div>` : '';
}
...
<title>${esc(title)}</title>${fontsHref ? `\n<link rel="stylesheet" href="${fontsHref}">` : ''}
<style>
${rootCss}
...
const foot = (d: DashboardData) => `<footer>${esc(d.disclaimer ?? 'Educational longevity analysis. Not a diagnosis or medical advice. Confirm high-stakes findings with a clinician.')}</footer></div></body></html>`;

// ---- SVG building blocks ----

// Gradient progress ring with a soft glow and animated fill.
function ringSVG(score: number, opts: { size?: number; stroke?: number; id: string; from: string; to: string; track?: string; glow?: boolean } ): string {
  const size = opts.size ?? 240, sw = opts.stroke ?? 14, r = size / 2 - sw, circ = 2 * Math.PI * r;
  ...
  return `<svg class="ring" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" style="--circ:${circ};--off:${off}">
    <defs><linearGradient id="${opts.id}" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0" stop-color="${opts.from}"/><stop offset="1" stop-color="${opts.to}"/></linearGradient>
      ${opts.glow ? `<filter id="${opts.id}-g" x="-30%" y="-30%" width="160%" height="160%"><feGaussianBlur stdDeviation="6" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>` : ''}</defs>
    <circle cx="${size / 2}" cy="${size / 2}" r="${r}" fill="none" stroke="${opts.track ?? 'var(--border)'}" stroke-width="${sw}"/>
    <circle cx="${size / 2}" cy="${size / 2}" r="${r}" fill="none" stroke="url(#${opts.id})" stroke-width="${sw}" stroke-linecap="round"
      stroke-dasharray="${circ}" stroke-dashoffset="${off}" transform="rotate(-90 ${size / 2} ${size / 2})"
      ${opts.glow ? `filter="url(#${opts.id}-g)"` : ''} style="animation:ringFill 1.4s cubic-bezier(.22,1,.36,1) both"/>
  </svg>`;
}

// Semicircular gauge with tick marks and a gradient arc.
function gaugeSVG(label: string, score: number, id: string, stops: string[]): string {
  const w = 220, h = 130, cx = w / 2, cy = 118, r = 92, sw = 16;
  const a0 = Math.PI, a1 = Math.PI - (Math.PI * clampPct(score)) / 100;
  const p = (ang: number, rad: number) => `${cx + rad * Math.cos(ang)} ${cy - rad * Math.sin(ang)}`;
  const arc = (fromA: number, toA: number, rad: number, large = 0) => `M ${p(fromA, rad)} A ${rad} ${rad} 0 ${large} 1 ${p(toA, rad)}`;
  const ticks = Array.from({ length: 11 }, (_, i) => {
    const ang = Math.PI - (Math.PI * i) / 10;
    return `<line x1="${cx + (r + 10) * Math.cos(ang)}" y1="${cy - (r + 10) * Math.sin(ang)}" x2="${cx + (r + 2) * Math.cos(ang)}" y2="${cy - (r + 2) * Math.sin(ang)}" stroke="var(--border)" stroke-width="2"/>`;
  }).join('');
  ...
  return `<div class="gauge"><svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">
    ...
    ${ticks}
    <path d="${arc(Math.PI, 0, r, 1)}" fill="none" stroke="var(--surface-alt)" stroke-width="${sw}" stroke-linecap="round"/>
    <path d="${arc(a0, a1, r, 0)}" fill="none" stroke="url(#${id})" stroke-width="${sw}" stroke-linecap="round" style="animation:fadeIn 1.2s ease both"/>
  </svg><div class="gauge-val">${num(score)}</div><div class="gauge-label">${esc(label)}</div></div>`;
}

// Small donut for a percentage (in-range, etc.).
function donutSVG(pct: number, color: string): string {
  const s = 84, r = 34, sw = 9, circ = 2 * Math.PI * r, off = circ - (circ * clampPct(pct)) / 100;
  return `<svg width="${s}" height="${s}" viewBox="0 0 ${s} ${s}">
    ...
    <circle cx="${s / 2}" cy="${s / 2}" r="${r}" fill="none" stroke="${color}" stroke-width="${sw}" stroke-linecap="round"
      stroke-dasharray="${circ}" stroke-dashoffset="${off}" transform="rotate(-90 ${s / 2} ${s / 2})"/>
    <text x="${s / 2}" y="${s / 2 + 5}" text-anchor="middle" font-size="20" font-weight="700" fill="var(--text)">${Math.round(pct)}</text></svg>`;
}

// Deterministic little sparkline from a seed value (illustrative trend).
function sparkline(seed: number, color: string): string {
  const w = 92, h = 28, n = 12;
  let x = seed * 7.13;
  const pts = Array.from({ length: n }, (_, i) => {
    x = (Math.sin(x) + 1) / 2; return `${(i / (n - 1)) * w},${h - (0.2 + x * 0.6) * h}`;
  }).join(' ');
...
function renderRingData(d: DashboardData, theme: DesignTokens, v: DesignVoice): string {
  ...
  const contrib = d.cards.slice(0, 3).map(c => `<div class="c">
    ${ringSVG(c.score ?? 0, { size: 92, stroke: 8, id: `c-${esc(c.title).replace(/\W/g, '')}`, from: statusColor(c.status), to: 'var(--accent)' })}
    <div class="cn">${num(c.score)}</div><div class="cl">${esc(c.title)}</div></div>`).join('');
  const plan = (d.priorities ?? d.cards.slice(0, 3).map(c => ({ title: c.action ?? c.summary ?? c.title }))).slice(0, 3)
    .map((p, i) => `<li><b>0${i + 1}</b><span>${esc(p.title)}</span></li>`).join('');
  return head(theme, `${v.scoreWord} today`, css) +
    `<section class="hero"><p class="kicker">${esc(v.heroKicker)}</p>
      <div class="ring-wrap">${ringSVG(d.score ?? 0, { size: 244, stroke: 14, id: 'hero', from: 'var(--primary)', to: 'var(--accent)', glow: true })}
        <div class="ring-num"><b>${num(d.score)}</b><s>${esc(v.scoreWord)}</s></div></div>
      <h1>Your ${v.scoreWord.toLowerCase()} is ${d.score != null && d.score >= 70 ? 'balanced' : 'worth a look'}</h1>
      ${d.summary ? `<p>${esc(d.summary)}</p>` : ''}
      <div style="margin-top:18px">${coverageStrip(d)}</div></section>
    <div class="contrib">${contrib}</div>
    <section class="plan"><h2>${esc(v.planTitle)}</h2><ul>${plan}</ul></section>` + foot(d);
}

// ==== performance (WHOOP): bold coach, dual tick-marked gauges, metric bars ====
function renderPerformance(d: DashboardData, theme: DesignTokens, v: DesignVoice): string {
  ...
  const bars = d.cards.slice(0, 8).map(c => `<div class="bar"><span class="n">${esc(c.title)}</span>
    <div class="t"><i style="width:${clampPct(c.score)}%;background:${statusColor(c.status)}"></i></div><span class="v">${num(c.score)}</span></div>`).join('');
  const plan = (d.priorities ?? d.cards.filter(c => c.action).slice(0, 3).map(c => ({ title: c.action!, why: c.summary })))
    .slice(0, 3).map(p => `<div class="p"><div class="pt">${esc(p.title)}</div>${(p as any).why ? `<div class="pw">${esc((p as any).why)}</div>` : ''}</div>`).join('');
  return head(theme, `${v.scoreWord}`, css) +
    `<p class="kicker">${esc(v.heroKicker)}</p><div class="band"></div>
    <section class="hero">
      ${gaugeSVG('Recovery', d.score ?? 0, 'g-rec', ['var(--negative)', 'var(--warning)', 'var(--positive)'])}
      ${gaugeSVG('Strain', d.cards[0]?.score ?? 0, 'g-str', ['var(--accent)', 'var(--primary)'])}
    </section>${coverageStrip(d)}
    <section class="bars">${bars}</section>
    <section class="plan"><h2>${esc(v.planTitle)}</h2>${plan}</section>` + foot(d);
}

// ==== clinical-modern (Superpower): lab panel, donut, reference-range table ====
function renderClinical(d: DashboardData, theme: DesignTokens, v: DesignVoice): string {
  const flagged = d.cards.filter(c => ['watch', 'needs_attention', 'high', 'low', 'out_of_range', 'borderline'].includes(String(c.status).toLowerCase())).length;
  ...
  const rows = d.cards.slice(0, 12).map((c, i) => `<tr style="animation-delay:${i * 40}ms"><td class="mk">${esc(c.title)}</td>
    <td class="val">${num(c.score)}</td>
    <td class="range"><div class="t"><div class="band"></div><div class="dot" style="left:calc(${clampPct(c.score)}% - 7px);background:${statusColor(c.status)}"></div></div></td>
    <td><span class="pill" style="color:${statusColor(c.status)};background:color-mix(in srgb,${statusColor(c.status)} 14%,transparent)">${esc(String(c.status ?? '').replace(/_/g, ' '))}</span></td></tr>`).join('');
  const recs = (d.priorities ?? d.cards.filter(c => c.action).slice(0, 4).map(c => ({ title: c.action! })))
    .slice(0, 4).map((p, i) => `<li><b>0${i + 1}</b><span>${esc(p.title)}</span></li>`).join('');
  return head(theme, `${v.scoreWord} panel`, css) +
    `<div class="top"><div><p class="kicker">${esc(v.heroKicker)}</p><h1>${v.scoreWord} panel</h1>
      <div class="sub">${esc(d.summary ?? `${d.cards.length} markers reviewed.`)}</div><div style="margin-top:10px">${coverageStrip(d)}</div></div>
      <div class="donut">${donutSVG(pct, 'var(--positive)')}<div class="dl">${inRange} of ${d.cards.length} markers in range</div></div></div>
    <table><thead><tr><th>Marker</th><th>Value</th><th>Reference range</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table>
    <section class="rec"><h2>${esc(v.planTitle)}</h2><ul>${recs}</ul></section>` + foot(d);
}

// ==== metabolic (Levels): glucose-style zone curve, time-in-range, chips ====
function renderMetabolic(d: DashboardData, theme: DesignTokens, v: DesignVoice): string {
  const inRange = d.cards.filter(c => ['optimal', 'good', 'ok', 'in_range'].includes(String(c.status).toLowerCase())).length;
  ...
  const W = 640, H = 150, seed = (d.score ?? 60) * 0.09;
  const pts = Array.from({ length: 40 }, (_, i) => {
    const t = i / 39;
    const y = 0.46 + 0.15 * Math.sin(t * 6.2 + seed) + 0.06 * Math.sin(t * 15 + seed * 2);
  ...
  const chips = d.cards.slice(0, 10).map(c => `<span class="chip"><span class="dot" style="background:${statusColor(c.status)}"></span>${esc(c.title)} <b>${num(c.score)}</b></span>`).join('');
  const plan = (d.priorities ?? d.cards.filter(c => c.action).slice(0, 3).map(c => ({ title: c.action!, why: c.summary })))
    .slice(0, 3).map(p => `<div class="p"><b>${esc(p.title)}</b>${(p as any).why ? `<div class="w">${esc((p as any).why)}</div>` : ''}</div>`).join('');
  return head(theme, `${v.scoreWord} signal`, css) +
    `<section class="hero"><p class="kicker">${esc(v.heroKicker)}</p>
      <div class="headline"><div class="metascore">${num(d.score)}</div>
        <div class="tir">${donutSVG(pct, 'var(--positive)')}<div class="tl">${pct}% of markers in range</div></div></div>
...
function renderSystemCards(d: DashboardData, theme: DesignTokens, v: DesignVoice): string {
  ...
  const cards = d.cards.slice(0, 9).map((c, i) => `<div class="card" style="animation-delay:${i * 45}ms">
    <div class="h"><div class="icon" style="background:${statusColor(c.status)}">${esc(String(c.title)[0] ?? 'H')}</div><div class="ct">${esc(c.title)}</div></div>
    <div class="row"><div class="cv">${num(c.score)}</div>${sparkline((c.score ?? 50) + i, statusColor(c.status))}</div>
    <div class="cs" style="color:${statusColor(c.status)}">${esc(String(c.status ?? '').replace(/_/g, ' '))}</div>
    ${c.summary ? `<div class="cm">${esc(c.summary)}</div>` : ''}</div>`).join('');
  const plan = (d.priorities ?? d.cards.filter(c => c.action).slice(0, 4).map(c => ({ title: c.action! })))
    .slice(0, 4).map(p => `<div class="r">${esc(p.title)}</div>`).join('');
  return head(theme, `${v.scoreWord}`, css) +
    `<div class="top"><p class="kicker">${esc(v.heroKicker)} · ${v.scoreWord} ${num(d.score)}</p><h1>Your ${v.scoreWord.toLowerCase()}</h1>
...
function renderSerene(d: DashboardData, theme: DesignTokens, v: DesignVoice): string {
  ...
  const top = d.cards[0];
  const plan = (d.priorities ?? d.cards.slice(0, 2).map(c => ({ title: c.action ?? c.summary ?? c.title })))
    .slice(0, 2).map(p => `<div class="p">${esc(p.title)}</div>`).join('');
  return head(theme, `${v.scoreWord}`, css) +
    `<section class="hero"><p class="k">${esc(v.heroKicker)}</p>
      <div class="orbwrap"><div class="orb"></div><div class="orb-num">${num(d.score)}</div></div>
      <h1>Take a breath.<br>Your ${v.scoreWord.toLowerCase()} is ${d.score != null && d.score >= 70 ? 'steady' : 'asking for attention'}.</h1>
      ${d.summary ? `<p>${esc(d.summary)}</p>` : ''}${coverageStrip(d)}</section>
    ${top ? `<p class="insight">${esc(top.summary ?? `${top.title} is the signal to sit with today.`)}</p>` : ''}
    <section class="plan"><h2>${esc(v.planTitle)}</h2>${plan}</section>` + foot(d);
}

const RENDERERS: Record<string, (d: DashboardData, t: DesignTokens, v: DesignVoice) => string> = {
...
export function renderDesignDashboard(data: DashboardData, design?: string): string {
...
// The six summary-engine designs (each has a self-contained renderer above).
export const DESIGN_IDS = Object.keys(RENDERERS);

// `dossier` is the full "Healthspan dossier" dashboard — the original house
// layout, white-labeled. It does NOT render through the summary engine; locally
// it IS the deep template (see scripts/pipeline/index.ts, which writes that deep
// render to index.html when --design=dossier). It's listed here so callers can
// offer it as a design choice alongside the six summary layouts.
export const FULL_DASHBOARD_DESIGN = 'dossier';

// Every selectable design id, including the full-dashboard `dossier`.
...
// Designs with a thorough, all-modality renderer use it (performance so far);
// the others derive the minimal shape and use their layout until upgraded.
// Note: `dossier` is rendered upstream in the pipeline (index.html = the deep
...
export function pipelineToDashboardData(d: any): DashboardData {
  const cards = (d?.tracking ?? []).map((t: any) => ({
    title: t.title, score: t.score, status: t.scoreStatus ?? t.status, summary: t.summary, category: t.id,
  }));
